...
const { evaluateRun, readJson, verifyLock } = require('./backtest-core-v190.cjs');

const lockPath = process.argv[2];
const pricesPath = process.argv[3];
if (!lockPath || !pricesPath) {
  console.error('Usage: node scripts/evaluate-skill-run-v190.cjs <lock.json> <prices.json> [outcome.json]');
  process.exit(2);
}
try {
  ...
  const output = {
    schemaVersion: '1.1',
    evaluatorVersion: '1.9-fresh-incremental-escalation',
    runId: lock.runId,
