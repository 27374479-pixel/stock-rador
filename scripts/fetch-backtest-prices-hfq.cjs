...
const { readJson, verifyLock } = require('./backtest-core-v130.cjs');
...
async function fetchJson(url) {
  const response = await fetch(url, {
    headers: { 'User-Agent': 'stock-rador-backtest-hfq/1.3' },
...
async function main() {
  ...
  if (!lockPath || !outputPath) {
    console.error('Usage: node scripts/fetch-backtest-prices-hfq.cjs <lock.json> <prices.json>');
    process.exit(2);
  }
  ...
  for (const ticker of tickers) {
    ...
    const adjustedUrl = tencentUrl(symbol, start, end, 'hfq');
    ...
    if (isBenchmark) {
      series[ticker] = {
        adjusted,
        source: { provider: 'Tencent', adjustedUrl, adjustedSeriesKey: adjustedResponse?.data?.[symbol]?.hfqday ? 'hfqday' : 'day' }
      };
      continue;
    }
    ...
    series[ticker] = {
      adjusted,
      unadjusted,
      source: { provider: 'Tencent', adjustedUrl, unadjustedUrl, adjustedSeriesKey: 'hfqday', unadjustedSeriesKey: 'day' }
    };
  }
  ...
  const output = {
    schemaVersion: '1.0',
    runId: lock.runId,
    provider: 'Tencent public daily hfq + raw kline',
    adjustmentPolicy: 'hfq adjusted series is used for return paths to avoid negative old qfq levels after later large distributions; raw unadjusted bars remain frozen for execution checks',
    fetchedAt,
    revealGuard: {
