...
function validateOpportunityMemo(memo, memoPath, manifest) {
  ...
  if (['1.3', '1.4'].includes(manifest?.schemaVersion) && memo?.selectionState !== 'No selection') {
    ...
    if (!burden || typeof burden !== 'object') {
      ...
      const primary = manifest.primarySelectionStates.includes(memo.selectionState);
      const v07ResetSubstitute = ['0.7.0', '0.8.0', '0.9.0', '1.0.0'].includes(manifest?.skill?.version) &&
  ...
  if (['0.7.0', '0.8.0', '0.9.0', '1.0.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
  ...
  if (['0.8.0', '0.9.0', '1.0.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
    ...
    if (!asym || typeof asym !== 'object') {
      ...
      if (primary) {
        const v09TurningPoint = ['0.9.0', '1.0.0'].includes(manifest?.skill?.version) &&
  ...
  if (['0.9.0', '1.0.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
  ...
  if (manifest?.skill?.version === '1.0.0' && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
    ...
    if (memo?.timingRoute === 'company_engine_override') {
      if (!override || typeof override !== 'object') {
        ...
        if (!dislocation || typeof dislocation !== 'object') {
          ...
          if (!['de_rated', 'mixed', 'not_dislocated'].includes(dislocation.conclusion)) {
