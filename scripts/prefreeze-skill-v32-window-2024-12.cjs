...
const P=JSON.parse(fs.readFileSync(path.join(__dirname,'..','research','skill_v32_2024_12_research.json'),'utf8'));
const OUT=path.join(__dirname,'..','research','skill_v32_2024_12_prefreeze.json');
const Q=[
...
const F={
"002922.SZ":{sharesBn:0.3925,q3NetProfitBn:0.210439},
"002155.SZ":{sharesBn:1.202039474,q3NetProfitBn:0.666826},
"601020.SH":{sharesBn:0.819964698,q3NetProfitBn:0.152965}
};
...
async function main(){
 ...
 for(const s of out.signals)for(const c of s.candidates??[]){
  const [q,r]=await Promise.all([fetchJson(url(c.symbol,'2024-01-01',s.signalDate,'qfq')),fetchJson(url(c.symbol,'2024-01-01',s.signalDate,'none'))]);
  ...
  const close=rr.filter(x=>x.date<=s.signalDate).at(-1).close, f=F[c.ticker];
  const mc=close*f.sharesBn, ann=f.q3NetProfitBn*4/3;
  c.preSignalPrice={maxFetchedDate:s.signalDate,close:round(close,3),prior20Return:round(trailing(qr,s.signalDate,20)),prior60Return:round(trailing(qr,s.signalDate,60)),prior180Return:round(trailing(qr,s.signalDate,180)),valuation:{marketCapBn:round(mc,2),simpleAnnualizedQ3PE:round(mc/ann,2),warning:'Simple Q1-Q3 annualization is context only, not a forecast.'}};
 }
