...
const CONFIG_PATH = path.join(__dirname, '..', 'config', 'v21_window_2022_06.json');
const OUTPUT_JSON = path.join(__dirname, '..', 'research', 'v21_2022_06_selection.json');
const OUTPUT_MD = path.join(__dirname, '..', 'research', 'v21_2022_06_selection.md');
...
async function main() {
  ...
  const leads = CONFIG.leads.map((lead) => {
    const asOf = `${lead.signalDate}T23:59:59.000Z`;
    ...
    return {
      ...
      evidence: lead.evidence,
      reversalEvidence: lead.reversalEvidence ?? [],
      candidates,
...
`;
  ...
  fs.writeFileSync(OUTPUT_MD, `# V2.1 June 2022 selection freeze

**No post-signal price data is present.** Every market-data request ends on or before the relevant signal date. V2.1 was frozen before this window's outcome retrieval.
...
${sections}`);
  ...
  console.log(JSON.stringify(leads.map((lead) => ({
    ...
    candidates: lead.candidates.map((candidate) => ({
      ...
      reversalRisk: candidate.score.reversalRiskPenalty
    }))
  })), null, 2));
}
