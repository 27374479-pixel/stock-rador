...
function validateOpportunityMemo(memo, memoPath, manifest) {
  ...
  if (['1.3', '1.4'].includes(manifest?.schemaVersion) && memo?.selectionState !== 'No selection') {
    ...
    if (!burden || typeof burden !== 'object') {
      ...
      const primary = manifest.primarySelectionStates.includes(memo.selectionState);
      const v07ResetSubstitute = ['0.7.0', '0.8.0'].includes(manifest?.skill?.version) &&
  ...
  if (['0.7.0', '0.8.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
  ...
  if (manifest?.skill?.version === '0.8.0' && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
    ...
    if (!asym || typeof asym !== 'object') {
      ...
      if (primary) {
        if (q?.conclusion !== 'pass') {
          errors.push(`${memoPath} High-priority selection requires qualityFloor pass`);
        }
