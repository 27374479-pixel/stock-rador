...
function parseTencent(response, symbol, allowIndexFallback = false) {
  ...
  const rows = node?.qfqday ?? (allowIndexFallback ? node?.day : undefined);
...
async function fetchJson(url) {
  const response = await fetch(url, {
    headers: { 'User-Agent': 'stock-rador-regime/0.8' },
...
async function main() {
  ...
  for (const ticker of tickers) {
    ...
    const url = `https://web.ifzq.gtimg.cn/appstock/app/fqkline/get?param=${symbol},day,${start},${config.cutoffDate},300,qfq`;
  ...
  const output = {
    ...
    provider: 'Tencent public daily qfq kline',
    benchmarkTicker,
