...
function validateFreshIncrementalSourceLink(memo, memoPath, sourcePack, manifest) {
  const errors = [];
  ...
  if (!test || test.applicable !== true) return errors;

  const eventClusters = Array.isArray(sourcePack?.eventClusters) ? sourcePack.eventClusters : [];
  const event = eventClusters.find((x) => x?.eventId === test.eventId);
  if (!event) {
    errors.push(memoPath + ' fresh incremental eventId is missing from frozen source pack: ' + (test.eventId ?? 'unknown'));
    return errors;
  }
  if (event.noveltyAssessment !== test.discoveryNovelty) {
    errors.push(memoPath + ' fresh incremental discoveryNovelty does not match frozen event cluster');
  }
  if (event.firstAvailableAt !== test.eventFirstAvailableAt) {
    errors.push(memoPath + ' fresh incremental eventFirstAvailableAt does not match frozen event cluster');
  }

  const positiveRoute = ['clear_route', 'credible_route'].includes(test.conclusion);
  if (positiveRoute && event.noveltyAssessment !== 'new') {
    errors.push(memoPath + ' positive v1.9 fresh escalation cannot link to a continuation event');
  }
  ...
  const sourceItems = Array.isArray(sourcePack?.sourceItems) ? sourcePack.sourceItems : [];
  const sources = new Map();
  for (const item of [...reviewItems, ...evidenceDocuments, ...sourceItems]) {
    const id = item?.itemId ?? item?.id;
    if (id) sources.set(id, item);
  }

  const freshRefs = Array.isArray(test.freshDriverEvidenceRefs) ? test.freshDriverEvidenceRefs : [];
  const baselineRefs = Array.isArray(test.baselineEvidenceRefs) ? test.baselineEvidenceRefs : [];
  const memberIds = new Set(Array.isArray(event.memberItemIds) ? event.memberItemIds : []);
  if (!freshRefs.some((id) => memberIds.has(id))) {
    errors.push(memoPath + ' fresh driver evidence must include a member source of the frozen event cluster');
  }

  for (const id of freshRefs) {
    const source = sources.get(id);
    if (!source) {
      errors.push(memoPath + ' fresh driver evidence is missing from frozen source pack: ' + id);
      continue;
    }
    const available = source.availableAt ?? source.publishedAt ?? source.date;
    if (available && memo?.cutoffAt && Date.parse(available) > Date.parse(memo.cutoffAt)) {
      errors.push(memoPath + ' fresh driver evidence is post-cutoff: ' + id);
    }
  }
  for (const id of baselineRefs) {
    const source = sources.get(id);
    if (!source) {
      errors.push(memoPath + ' baseline evidence is missing from frozen source pack: ' + id);
      continue;
    }
    const available = source.availableAt ?? source.publishedAt ?? source.date;
    if (available && test.baselineAsOfAt && Date.parse(available) > Date.parse(test.baselineAsOfAt)) {
      errors.push(memoPath + ' baseline evidence was not available by baselineAsOfAt: ' + id);
    }
  }
  return errors;
}
...
function isV19FreshEscalation(memo, manifest) {
  return manifest?.skill?.version === '1.9.0' &&
    memo?.timingRoute === 'base_horizon_convexity' &&
    memo?.freshIncrementalEscalationTest?.applicable === true &&
...
function validateOpportunityMemo(memo, memoPath, manifest) {
  ...
  if (['1.3', '1.4'].includes(manifest?.schemaVersion) && memo?.selectionState !== 'No selection') {
    ...
    if (!burden || typeof burden !== 'object') {
      ...
      const primary = manifest.primarySelectionStates.includes(memo.selectionState);
      const v07ResetSubstitute = ['0.7.0', '0.8.0', '0.9.0', '1.0.0', '1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version) &&
  ...
  if (['0.7.0', '0.8.0', '0.9.0', '1.0.0', '1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
  ...
  if (['1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
    ...
    if (!pathTest || typeof pathTest !== 'object') {
      ...
      if (primary) {
        const v15BaseHorizonRoute =
          ['1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version) &&
          memo?.timingRoute === 'base_horizon_convexity';
  ...
  if (['1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const archetypes = [
    ...
    if (expressions.length < 1 || expressions.length > 3) {
      ...
      if (primary) {
        ...
        for (const expression of expressions.filter((x) => x?.conclusion === 'acceptable')) {
          ...
          const v19FreshSelectedExpression = isV19FreshEscalation(memo, manifest) &&
            expression.ticker === memo?.selectionComparison?.selectedTicker;
          const allowedExpectationFit = v19FreshSelectedExpression ? ['clear', 'credible', 'mixed'] : ['clear', 'credible'];
          const allowedExpressionDownside = v19FreshSelectedExpression ? ['clear', 'credible', 'mixed'] : ['clear', 'credible'];
          if (!allowedExpectationFit.includes(expression.expectationFit)) {
            errors.push(`${memoPath} High-priority acceptable expression ${expression.ticker} requires ${v19FreshSelectedExpression ? 'clear/credible/mixed under v1.9 fresh route' : 'clear/credible'} expectationFit`);
          }
          if (!allowedExpressionDownside.includes(expression.downsideContainment)) {
            errors.push(`${memoPath} High-priority acceptable expression ${expression.ticker} requires ${v19FreshSelectedExpression ? 'clear/credible/mixed under v1.9 fresh route' : 'clear/credible'} downsideContainment`);
          }
        }
      }
    }
  ...
  if (['1.4.0', '1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version)) {
  ...
  if (['0.8.0', '0.9.0', '1.0.0', '1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
    ...
    if (!timing || typeof timing !== 'object') {
      ...
      if (primary) {
        const v10EngineOverride =
          ['1.0.0', '1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version) &&
    ...
    if (!asym || typeof asym !== 'object') {
      ...
      if (primary) {
        const v09TurningPoint = ['0.9.0', '1.0.0', '1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version) &&
        ...
        const v19FreshEscalation = isV19FreshEscalation(memo, manifest);
        const allowedMemoDownside = v19FreshEscalation ? ['clear', 'credible', 'mixed'] : ['clear', 'credible'];
        if (!allowedMemoDownside.includes(asym?.downsideContainment?.conclusion)) {
          errors.push(`${memoPath} High-priority selection requires ${v19FreshEscalation ? 'clear/credible/mixed bounded' : 'clear/credible'} downside containment`);
        }
        if (['1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version)) {
  ...
  if (['0.9.0', '1.0.0', '1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
  ...
  if (['1.0.0', '1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);

    const v15BaseHorizonRoute = ['1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version) && memo?.timingRoute === 'base_horizon_convexity';
    const allowedTimingRoutes = ['1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version)
    ...
    if (memo?.timingRoute === 'company_engine_override') {
      if (!override || typeof override !== 'object') {
        ...
        if (!dislocation || typeof dislocation !== 'object') {
          ...
          const isV11 = ['1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version);
        ...
        if (primary) {
          ...
          const directionalV11Plus = ['1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version);
  ...
  if (['1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    ...
    if (isBaseRoute) {
      if (!test || typeof test !== 'object') {
        ...
        if (primary) {
          ...
          const v19FreshEscalation = isV19FreshEscalation(memo, manifest);
          const allowedBaseDownside = v19FreshEscalation ? ['clear', 'credible', 'mixed'] : ['clear', 'credible'];
          if (!allowedBaseDownside.includes(memo?.crossSectionalAsymmetryTest?.downsideContainment?.conclusion)) {
            errors.push(`${memoPath} High-priority base_horizon_convexity requires ${v19FreshEscalation ? 'clear/credible/mixed bounded' : 'clear/credible'} downside containment`);
          }
  ...
  if (['1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version)) {
  ...
  if (['1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const adversarial = memo?.adversarialEvidenceTest;
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
    const requiredAxes = [
    ...
    if (!adversarial || typeof adversarial !== 'object') {
      ...
      const axes = Array.isArray(adversarial.axes) ? adversarial.axes : [];
      if (axes.length === 0) {
        errors.push(memoPath + ' adversarialEvidenceTest.axes must be non-empty');
      }
      const seenAxes = new Set();
      for (const axis of axes) {
        if (!allowedAxes.has(axis?.axis)) {
          errors.push(memoPath + ' adversarialEvidenceTest axis is invalid: ' + (axis?.axis ?? 'unknown'));
          continue;
        }
        if (seenAxes.has(axis.axis)) {
          errors.push(memoPath + ' adversarialEvidenceTest duplicates axis ' + axis.axis);
        }
        seenAxes.add(axis.axis);
        if (!allowedAxisConclusions.includes(axis?.conclusion)) {
          errors.push(memoPath + ' adversarialEvidenceTest ' + axis.axis + ' conclusion is invalid');
        }
        if (typeof axis?.rationale !== 'string' || !axis.rationale.trim()) {
          errors.push(memoPath + ' adversarialEvidenceTest ' + axis.axis + ' rationale is required');
        }
        if (!Array.isArray(axis?.evidenceRefs) || axis.evidenceRefs.length === 0) {
          errors.push(memoPath + ' adversarialEvidenceTest ' + axis.axis + ' evidenceRefs must be non-empty');
        }
      }

      if (typeof adversarial.strongestCounterThesis !== 'string' || !adversarial.strongestCounterThesis.trim()) {
        errors.push(memoPath + ' adversarialEvidenceTest.strongestCounterThesis is required');
      }
      if (!['low', 'moderate', 'high', 'fatal'].includes(adversarial.counterThesisSeverity)) {
        errors.push(memoPath + ' adversarialEvidenceTest.counterThesisSeverity is invalid');
      }
      if (!['none', 'tighten_downside', 'downgrade_quality', 'block_primary'].includes(adversarial.thesisImpact)) {
        errors.push(memoPath + ' adversarialEvidenceTest.thesisImpact is invalid');
      }
      if (typeof adversarial.falsifierOrEscalationTrigger !== 'string' || !adversarial.falsifierOrEscalationTrigger.trim()) {
        errors.push(memoPath + ' adversarialEvidenceTest.falsifierOrEscalationTrigger is required');
      }
      if (!['clear_to_proceed', 'proceed_with_caveats', 'research_only', 'reject'].includes(adversarial.conclusion)) {
        errors.push(memoPath + ' adversarialEvidenceTest.conclusion is invalid');
      }

      if (['high', 'fatal'].includes(adversarial.counterThesisSeverity) &&
          !['research_only', 'reject'].includes(adversarial.conclusion)) {
        errors.push(memoPath + ' high/fatal adversarial counter-thesis cannot conclude proceed');
      }
      if (adversarial.thesisImpact === 'block_primary' &&
          !['research_only', 'reject'].includes(adversarial.conclusion)) {
        errors.push(memoPath + ' block_primary adversarial impact cannot conclude proceed');
      }

      if (primary) {
        if (adversarial.searchCompleteness !== 'complete') {
          errors.push(memoPath + ' High-priority selection requires complete adversarial search');
        }
        for (const requiredAxis of requiredAxes) {
          const axis = axes.find((item) => item?.axis === requiredAxis);
          if (!axis) {
            errors.push(memoPath + ' High-priority selection requires adversarial axis ' + requiredAxis);
            continue;
          }
          if (!['clear', 'credible_risk'].includes(axis.conclusion)) {
            errors.push(memoPath + ' High-priority adversarial axis ' + requiredAxis + ' must be clear or credible_risk');
          }
        }
        if (!['low', 'moderate'].includes(adversarial.counterThesisSeverity)) {
          errors.push(memoPath + ' High-priority selection cannot have high/fatal adversarial counter-thesis');
        }
        if (!['none', 'tighten_downside'].includes(adversarial.thesisImpact)) {
          errors.push(memoPath + ' High-priority selection cannot downgrade quality or block Primary in adversarial test');
        }
        if (!['clear_to_proceed', 'proceed_with_caveats'].includes(adversarial.conclusion)) {
          errors.push(memoPath + ' High-priority selection requires adversarial clear_to_proceed/proceed_with_caveats');
        }
        if (adversarial.conclusion === 'clear_to_proceed' &&
            (adversarial.counterThesisSeverity !== 'low' || adversarial.thesisImpact !== 'none')) {
          errors.push(memoPath + ' adversarial clear_to_proceed requires low severity and no thesis impact');
        }
        if (adversarial.conclusion === 'proceed_with_caveats' &&
            (adversarial.counterThesisSeverity !== 'moderate' || adversarial.thesisImpact !== 'tighten_downside')) {
          errors.push(memoPath + ' adversarial proceed_with_caveats requires moderate severity and tighten_downside');
        }
      }
    }
  }
  ...
  if (['1.8.0', '1.9.0'].includes(manifest?.skill?.version)) {
    ...
    if (['company_sector_conflict', 'multi_axis_conflict'].includes(memo?.evidenceHorizonMatrix?.conflictType)) {
      triggerSignals.push('evidence_horizon_' + memo.evidenceHorizonMatrix.conflictType);
    }
    if (memo?.causalStateVariableTest?.stateFlowRelation === 'conflicted_unresolved') {
      triggerSignals.push('state_flow_conflicted_unresolved');
    }
    const aggregateConflictTriggered = triggerSignals.length > 0;
    const allowedLaneNames = [
      'geography_regulatory_exposure',
      'compliance_capacity_allowance',
      'product_mix',
      'cost_curve_input_access',
      'contract_pricing_structure',
      'customer_endmarket_mix',
      'working_capital_cash_quality',
      'other_material_heterogeneity'
    ];

    if (!exceptionTest || typeof exceptionTest !== 'object') {
      errors.push(memoPath + ' v1.8 requires companyExceptionSearchTest');
    } else {
      if (typeof exceptionTest.applicable !== 'boolean') {
        errors.push(memoPath + ' companyExceptionSearchTest.applicable must be boolean');
      }

      if (exceptionTest.applicable === false) {
        if (aggregateConflictTriggered) {
          errors.push(memoPath + ' aggregate conflict requires applicable companyExceptionSearchTest');
        }
        if (exceptionTest.overallConclusion !== 'not_applicable') {
          errors.push(memoPath + ' non-applicable companyExceptionSearchTest must conclude not_applicable');
        }
        if (!Array.isArray(exceptionTest.triggerSignals) || exceptionTest.triggerSignals.length !== 0) {
          errors.push(memoPath + ' non-applicable companyExceptionSearchTest triggerSignals must be empty');
        }
      } else if (exceptionTest.applicable === true) {
        if (!aggregateConflictTriggered) {
          errors.push(memoPath + ' applicable companyExceptionSearchTest requires an aggregate-conflict trigger');
        }

        if (!Array.isArray(exceptionTest.triggerSignals) || exceptionTest.triggerSignals.length === 0 ||
            exceptionTest.triggerSignals.some((x) => typeof x !== 'string' || !x.trim())) {
          errors.push(memoPath + ' applicable companyExceptionSearchTest requires non-empty triggerSignals');
        } else {
          for (const signal of triggerSignals) {
            if (!exceptionTest.triggerSignals.includes(signal)) {
              errors.push(memoPath + ' companyExceptionSearchTest is missing derived trigger signal ' + signal);
            }
          }
        }

        if (!['complete', 'partial', 'insufficient'].includes(exceptionTest.searchCompleteness)) {
          errors.push(memoPath + ' companyExceptionSearchTest.searchCompleteness is invalid');
        }

        const lanes = exceptionTest.heterogeneityLanes;
        if (!lanes || typeof lanes !== 'object' || Array.isArray(lanes)) {
          errors.push(memoPath + ' companyExceptionSearchTest.heterogeneityLanes is required');
        } else {
          for (const laneName of allowedLaneNames) {
            const lane = lanes[laneName];
            if (!lane || typeof lane !== 'object') {
              errors.push(memoPath + ' companyExceptionSearchTest missing lane ' + laneName);
              continue;
            }
            if (!['applicable', 'not_applicable'].includes(lane.applicability)) {
              errors.push(memoPath + ' companyExceptionSearchTest lane ' + laneName + ' applicability is invalid');
            }
            if (!['searched_no_exception', 'credible_exception', 'unresolved', 'not_applicable'].includes(lane.status)) {
              errors.push(memoPath + ' companyExceptionSearchTest lane ' + laneName + ' status is invalid');
            }
            if (lane.applicability === 'not_applicable' && lane.status !== 'not_applicable') {
              errors.push(memoPath + ' non-applicable companyExceptionSearchTest lane ' + laneName + ' must use not_applicable status');
            }
            if (lane.applicability === 'applicable' && lane.status === 'not_applicable') {
              errors.push(memoPath + ' applicable companyExceptionSearchTest lane ' + laneName + ' cannot use not_applicable status');
            }
            if (typeof lane.rationale !== 'string' || !lane.rationale.trim()) {
              errors.push(memoPath + ' companyExceptionSearchTest lane ' + laneName + ' rationale is required');
            }
            if (!Array.isArray(lane.searchTrace) || lane.searchTrace.some((x) => typeof x !== 'string' || !x.trim()) ||
                (lane.applicability === 'applicable' && lane.searchTrace.length === 0)) {
              errors.push(memoPath + ' applicable companyExceptionSearchTest lane ' + laneName + ' requires searchTrace');
            }
            if (!Array.isArray(lane.evidenceRefs) || lane.evidenceRefs.some((x) => typeof x !== 'string' || !x.trim())) {
              errors.push(memoPath + ' companyExceptionSearchTest lane ' + laneName + ' evidenceRefs must be a string array');
            }
            if (lane.status === 'credible_exception' && lane.evidenceRefs.length === 0) {
              errors.push(memoPath + ' credible companyExceptionSearchTest lane ' + laneName + ' requires evidenceRefs');
            }
          }
        }

        const candidates = Array.isArray(exceptionTest.candidateExceptions) ? exceptionTest.candidateExceptions : null;
        if (!candidates) {
          errors.push(memoPath + ' companyExceptionSearchTest.candidateExceptions must be an array');
        } else {
          const seen = new Set();
          for (const candidate of candidates) {
            if (typeof candidate?.ticker !== 'string' || !candidate.ticker.trim()) {
              errors.push(memoPath + ' companyExceptionSearchTest candidate requires ticker');
              continue;
            }
            if (seen.has(candidate.ticker)) {
              errors.push(memoPath + ' companyExceptionSearchTest duplicates candidate ' + candidate.ticker);
            }
            seen.add(candidate.ticker);
            if (typeof candidate.exceptionMechanism !== 'string' || !candidate.exceptionMechanism.trim()) {
              errors.push(memoPath + ' companyExceptionSearchTest candidate ' + candidate.ticker + ' requires exceptionMechanism');
            }
            for (const field of ['directExposure', 'earningsPathDifferentiation']) {
              if (!['clear', 'credible', 'mixed', 'insufficient'].includes(candidate[field])) {
                errors.push(memoPath + ' companyExceptionSearchTest candidate ' + candidate.ticker + ' ' + field + ' is invalid');
              }
            }
            if (!['research_candidate', 'not_actionable', 'reject'].includes(candidate.disposition)) {
              errors.push(memoPath + ' companyExceptionSearchTest candidate ' + candidate.ticker + ' disposition is invalid');
            }
            if (typeof candidate.rationale !== 'string' || !candidate.rationale.trim()) {
              errors.push(memoPath + ' companyExceptionSearchTest candidate ' + candidate.ticker + ' rationale is required');
            }
            if (!Array.isArray(candidate.evidenceRefs) || candidate.evidenceRefs.length === 0 ||
                candidate.evidenceRefs.some((x) => typeof x !== 'string' || !x.trim())) {
              errors.push(memoPath + ' companyExceptionSearchTest candidate ' + candidate.ticker + ' requires evidenceRefs');
            }
            if (candidate.disposition === 'research_candidate' &&
                !['clear', 'credible'].includes(candidate.directExposure)) {
              errors.push(memoPath + ' companyExceptionSearchTest research candidate ' + candidate.ticker + ' requires clear/credible direct exposure');
            }
            if (candidate.disposition === 'research_candidate' &&
                !['clear', 'credible'].includes(candidate.earningsPathDifferentiation)) {
              errors.push(memoPath + ' companyExceptionSearchTest research candidate ' + candidate.ticker + ' requires clear/credible earnings-path differentiation');
            }
          }
        }

        if (!['no_listed_exception_found', 'exception_candidate_found', 'exceptions_rejected', 'unresolved'].includes(exceptionTest.overallConclusion)) {
          errors.push(memoPath + ' companyExceptionSearchTest.overallConclusion is invalid');
        }
        if (typeof exceptionTest.rationale !== 'string' || !exceptionTest.rationale.trim()) {
          errors.push(memoPath + ' companyExceptionSearchTest.rationale is required');
        }
        if (typeof exceptionTest.falsifierOrEscalationTrigger !== 'string' || !exceptionTest.falsifierOrEscalationTrigger.trim()) {
          errors.push(memoPath + ' companyExceptionSearchTest.falsifierOrEscalationTrigger is required');
        }

        const researchCandidates = (candidates ?? []).filter((x) => x?.disposition === 'research_candidate');
        if (exceptionTest.overallConclusion === 'exception_candidate_found' && researchCandidates.length === 0) {
          errors.push(memoPath + ' exception_candidate_found requires at least one research_candidate');
        }
        if (researchCandidates.length > 0 && exceptionTest.overallConclusion !== 'exception_candidate_found') {
          errors.push(memoPath + ' research_candidate requires exception_candidate_found overall conclusion');
        }

        if (memo?.selectionState === 'No selection' && aggregateConflictTriggered) {
          if (exceptionTest.searchCompleteness !== 'complete') {
            errors.push(memoPath + ' conflicted aggregate No selection requires complete company-exception search');
          }
          if (researchCandidates.length > 0) {
            errors.push(memoPath + ' conflicted aggregate No selection cannot terminate while a company exception remains Research candidate');
          }
          if (!['no_listed_exception_found', 'exceptions_rejected'].includes(exceptionTest.overallConclusion)) {
            errors.push(memoPath + ' conflicted aggregate No selection requires no_listed_exception_found or exceptions_rejected');
          }
        }
      }
    }
...
function validateMemoForSelection(memo, selection, manifest) {
  ...
  if (isV12 && memo?.selectionState !== 'No selection') {
    ...
    const isV13 = ['1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0'].includes(manifest?.skill?.version);
    ...
    if (comparison) {
      ...
      if (primary) {
        const v19FreshEscalation = isV19FreshEscalation(memo, manifest);
        const allowedSelectionEdges = v19FreshEscalation ? ['clear', 'credible', 'mixed'] : ['clear', 'credible'];
        if (!allowedSelectionEdges.includes(comparison.selectionEdgeConclusion)) {
          errors.push(`${selection.memoPath} High-priority selection requires ${v19FreshEscalation ? 'clear/credible/mixed under v1.9 fresh route' : 'clear or credible'} selection edge`);
        }
...
function lockRun(manifestPath, rootDir = process.cwd()) {
  ...
  if (['1.2', '1.3', '1.4'].includes(manifest.schemaVersion)) {
    ...
    for (const memoRelativePath of manifest.hypothesisMemoPaths) {
      ...
      const freshLinkErrors = validateFreshIncrementalSourceLink(memo, memoRelativePath, sourcePack, manifest);
      const allMemoErrors = [...memoErrors, ...evidenceErrors, ...freshLinkErrors];
