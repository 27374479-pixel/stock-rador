...
const { evaluateRun, readJson, verifyLock } = require('./backtest-core-v131.cjs');

const lockPath = process.argv[2];
const pricesPath = process.argv[3];
if (!lockPath || !pricesPath) {
  console.error('Usage: node scripts/evaluate-skill-run-v130.cjs <lock.json> <prices.json> [outcome.json]');
  process.exit(2);
}
...
try {
  ...
  const output = {
    schemaVersion: '1.1',
    evaluatorVersion: '1.3.1-opportunity-first-interior-suspension-safe',
    runId: lock.runId,
    evaluatedAt: new Date().toISOString(),
    frozenLockSha256: crypto.createHash('sha256').update(fs.readFileSync(lockPath)).digest('hex'),
    infrastructurePatch: {
      type: 'post_lock_evaluator_compatibility_fix',
      researchContractChanged: false,
      selectionChanged: false,
      rule: 'benchmark calendar still defines horizon; interior stock suspensions may reduce observed stock bars without invalidating the return path'
    },
    priceSource: {
