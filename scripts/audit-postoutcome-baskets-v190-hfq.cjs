...
const { readJson, verifyLock, returnAfterCost } = require('./backtest-core-v190.cjs');
...
async function main(){
  ...
  if(!lockPath||!configPath||!outPath) throw new Error('Usage: node audit-postoutcome-baskets-v190-hfq.cjs <lock.json> <config.json> <out.json>');
