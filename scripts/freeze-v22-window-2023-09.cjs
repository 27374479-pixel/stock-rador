...
const CONFIG_PATH = path.join(__dirname, '..', 'config', 'v22_window_2023_09.json');
const OUTPUT_JSON = path.join(__dirname, '..', 'research', 'v22_2023_09_selection.json');
const OUTPUT_MD = path.join(__dirname, '..', 'research', 'v22_2023_09_selection.md');
...
async function main() {
  ...
  const leads = CONFIG.leads.map((lead) => {
    ...
    const candidates = lead.candidates.map((candidate) => {
      ...
      b.score.earningsBridgeScore - a.score.earningsBridgeScore ||
  ...
  const output = {
    ...
    watchOutcomesMustRemainUnopened: true,
    contaminationGuard: {
    ...
    leads,
    priceInputs: Object.fromEntries([...symbols.keys()].map((symbol) => [symbol, {
      source: marketData[symbol].source,
  ...
  const sections = leads.map((lead) => {
    const rows = lead.candidates.length
      ? lead.candidates.map((candidate) =>
          `| ${candidate.name} | ${candidate.score.decision} | ${candidate.score.total} | ${candidate.score.exposureScore} | ${candidate.score.surpriseScore} | ${candidate.score.monetizationScore} | ${candidate.score.earningsBridgeScore} | ${candidate.score.pricedInPenalty} | ${candidate.score.reversalRiskPenalty} | ${pct(candidate.pricing.prior60Return)} | ${pct(candidate.pricing.prior180Return)} |`
        ).join('\n')
      : '| — | Watch | — | — | — | — | — | — | — | — | — |';
...
`;
  ...
  fs.writeFileSync(OUTPUT_MD, `# V2.2 September 2023 selection freeze

**No post-signal price data is present.** Every market-data request ends on or before the relevant signal date. Previously viewed-outcome tickers are rejected by code. Watch-candidate futures are deliberately left unopened.

V2.2 was frozen before this window and adds an incremental-earnings bridge to prevent broad thematic adjacency from being mistaken for company-level earnings capture.
...
${sections}`);
  ...
  console.log(JSON.stringify(leads.map((lead) => ({
    ...
    candidates: lead.candidates.map((candidate) => ({
      ...
      earningsBridge: candidate.score.earningsBridgeScore,
