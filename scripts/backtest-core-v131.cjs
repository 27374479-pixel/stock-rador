...
function validateOpportunityMemo(memo, memoPath, manifest) {
  ...
  if (['1.3', '1.4'].includes(manifest?.schemaVersion) && memo?.selectionState !== 'No selection') {
    ...
    if (!burden || typeof burden !== 'object') {
      ...
      const primary = manifest.primarySelectionStates.includes(memo.selectionState);
      const v07ResetSubstitute = ['0.7.0', '0.8.0', '0.9.0', '1.0.0', '1.1.0', '1.2.0', '1.3.0'].includes(manifest?.skill?.version) &&
  ...
  if (['0.7.0', '0.8.0', '0.9.0', '1.0.0', '1.1.0', '1.2.0', '1.3.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
  ...
  if (['1.2.0', '1.3.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    ...
    if (!pathTest || typeof pathTest !== 'object') {
      ...
      if (primary) {
        ...
        if (!['low', 'moderate'].includes(duration?.conclusion)) {
          errors.push(`${memoPath} High-priority selection requires low/moderate expectation-duration risk`);
        }
        if (!['clear_path_room', 'credible_path_room'].includes(pathTest.overallConclusion)) {
          errors.push(`${memoPath} High-priority selection requires clear/credible expectation-path room`);
        }
      }
    }
  }
  ...
  if (manifest?.skill?.version === '1.3.0' && memo?.selectionState !== 'No selection') {
    const archetypes = [
  ...
  if (['0.8.0', '0.9.0', '1.0.0', '1.1.0', '1.2.0', '1.3.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
    ...
    if (!timing || typeof timing !== 'object') {
      ...
      if (primary) {
        const v10EngineOverride =
          ['1.0.0', '1.1.0', '1.2.0', '1.3.0'].includes(manifest?.skill?.version) &&
        ...
        if (!v10EngineOverride &&
          ...
          errors.push(`${memoPath} High-priority sector-confirmed selection requires positive earnings-revision breadth`);
        }
        if (v10EngineOverride &&
    ...
    if (!asym || typeof asym !== 'object') {
      ...
      if (primary) {
        const v09TurningPoint = ['0.9.0', '1.0.0', '1.1.0', '1.2.0', '1.3.0'].includes(manifest?.skill?.version) &&
        ...
        if (manifest?.skill?.version === '1.3.0') {
  ...
  if (['0.9.0', '1.0.0', '1.1.0', '1.2.0', '1.3.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
  ...
  if (['1.0.0', '1.1.0', '1.2.0', '1.3.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);

    if (!['sector_confirmed', 'company_engine_override', 'research_only'].includes(memo?.timingRoute)) {
      errors.push(`${memoPath} timingRoute must be sector_confirmed, company_engine_override, or research_only`);
    }
    ...
    if (memo?.timingRoute === 'company_engine_override') {
      if (!override || typeof override !== 'object') {
        ...
        if (!dislocation || typeof dislocation !== 'object') {
          ...
          const isV11 = ['1.1.0', '1.2.0', '1.3.0'].includes(manifest?.skill?.version);
        ...
        if (primary) {
          ...
          const directionalV11Plus = ['1.1.0', '1.2.0', '1.3.0'].includes(manifest?.skill?.version);
...
function validateMemoForSelection(memo, selection, manifest) {
  ...
  if (isV12 && memo?.selectionState !== 'No selection') {
    ...
    const isV13 = manifest?.skill?.version === '1.3.0';
...
function evaluateRun(manifest, memosByPath, prices) {
  ...
  for (const selection of manifest.selections) {
    ...
    for (const holdingDays of horizons) {
      ...
      // v1.3.1 infrastructure-only fix: a stock can be suspended on an interior market
      // trading day while still having valid entry and scheduled-exit bars. The benchmark
      // continues to define the pre-registered horizon; drawdown uses only observable
      // traded closes. Do not fabricate/forward-fill a suspended-day price.
      if (pathRows.length < 1 || pathRows.length > holdingDays) {
        throw new Error(`${selection.ticker} has invalid traded-path length ${pathRows.length} for benchmark horizon ${holdingDays}`);
      }
      ...
      outcome[String(holdingDays)] = {
        ...
        rankPercentile,
        controlReturns,
        benchmarkHoldingTradingDays: holdingDays,
        observedInstrumentTradingDays: pathRows.length,
        interiorSuspensionDays: holdingDays - pathRows.length,
