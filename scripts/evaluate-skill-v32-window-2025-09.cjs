...
const LOCK=JSON.parse(fs.readFileSync(path.join(__dirname,'..','frozen','skill_v32_2025_09_selection.json'),'utf8'));
const OUT_JSON=path.join(__dirname,'..','research','skill_v32_2025_09_outcomes.json');
const OUT_MD=path.join(__dirname,'..','research','skill_v32_2025_09_outcomes.md');

const H=[20,60,120,250];
const PRICE_START='2025-08-01';
const PRICE_END='2026-09-22';
const COST=0.001;
const BENCH={symbol:'sh000300',ticker:'000300.SH',name:'沪深300'};
const META={'603379.SH':{symbol:'sh603379',name:'三美股份',limitRate:0.10}};
...
async function main(){
  ...
  if(selected.length!==1||selected[0].ticker!=='603379.SH')throw new Error('unexpected frozen candidate set');

  const m=META['603379.SH'];
  ...
  const out={schemaVersion:'3.2',windowId:LOCK.windowId,generatedAt:new Date().toISOString(),
    selectionChangedAfterOutcome:false,watchOutcomesOpened:false,expectedResearchHorizon:'120-250 trading days',
  ...
  fs.writeFileSync(OUT_MD,`# Skill V3.2 September 2025 outcome evaluation

Only the frozen Candidate was fetched. 巨化股份 remains outcome-unopened.
