...
function validateOpportunityMemo(memo, memoPath, manifest) {
  ...
  if (['1.3', '1.4'].includes(manifest?.schemaVersion) && memo?.selectionState !== 'No selection') {
    ...
    if (!burden || typeof burden !== 'object') {
      ...
      const primary = manifest.primarySelectionStates.includes(memo.selectionState);
      const v07ResetSubstitute = ['0.7.0', '0.8.0', '0.9.0'].includes(manifest?.skill?.version) &&
  ...
  if (['0.7.0', '0.8.0', '0.9.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
  ...
  if (['0.8.0', '0.9.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
    ...
    if (!timing || typeof timing !== 'object') {
      ...
      if (primary) {
        if (timing.overallConclusion !== 'favorable') {
          errors.push(`${memoPath} High-priority selection requires favorable opportunity timing`);
        }
        if (timing?.priceRegime?.conclusion !== 'favorable') {
          errors.push(`${memoPath} High-priority selection requires favorable pre-cutoff industry price regime`);
        }
        ...
        if (!['broad_positive', 'narrow_positive'].includes(timing?.earningsRevisionBreadth?.conclusion)) {
          errors.push(`${memoPath} High-priority selection requires positive earnings-revision breadth`);
        }
    ...
    if (!asym || typeof asym !== 'object') {
      ...
      if (primary) {
        const v09TurningPoint = manifest?.skill?.version === '0.9.0' &&
  ...
  if (manifest?.skill?.version === '0.9.0' && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
