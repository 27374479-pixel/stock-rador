...
function validateOpportunityMemo(memo, memoPath, manifest) {
  ...
  if (['1.3', '1.4'].includes(manifest?.schemaVersion) && memo?.selectionState !== 'No selection') {
    ...
    if (!burden || typeof burden !== 'object') {
      ...
      const primary = manifest.primarySelectionStates.includes(memo.selectionState);
      const v07ResetSubstitute = ['0.7.0', '0.8.0', '0.9.0', '1.0.0', '1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version) &&
  ...
  if (['0.7.0', '0.8.0', '0.9.0', '1.0.0', '1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
  ...
  if (['1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
    ...
    if (!pathTest || typeof pathTest !== 'object') {
      ...
      if (primary) {
        const v15BaseHorizonRoute =
          ['1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version) &&
  ...
  if (['1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const archetypes = [
  ...
  if (['1.4.0', '1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version)) {
  ...
  if (['0.8.0', '0.9.0', '1.0.0', '1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
    ...
    if (!timing || typeof timing !== 'object') {
      ...
      if (primary) {
        const v10EngineOverride =
          ['1.0.0', '1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version) &&
    ...
    if (!asym || typeof asym !== 'object') {
      ...
      if (primary) {
        const v09TurningPoint = ['0.9.0', '1.0.0', '1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version) &&
        ...
        if (['1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version)) {
  ...
  if (['0.9.0', '1.0.0', '1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
  ...
  if (['1.0.0', '1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);

    const v15BaseHorizonRoute = ['1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version) && memo?.timingRoute === 'base_horizon_convexity';
    const allowedTimingRoutes = ['1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version)
    ...
    if (memo?.timingRoute === 'company_engine_override') {
      if (!override || typeof override !== 'object') {
        ...
        if (!dislocation || typeof dislocation !== 'object') {
          ...
          const isV11 = ['1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version);
        ...
        if (primary) {
          ...
          const directionalV11Plus = ['1.1.0', '1.2.0', '1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version);
  ...
  if (['1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
  ...
  if (manifest?.skill?.version === '1.7.0' && memo?.selectionState !== 'No selection') {
    const riskTest = memo?.adversarialCompanyRiskTest;
    if (!riskTest || typeof riskTest !== 'object') {
      errors.push(`${memoPath} v1.7 selected company requires adversarialCompanyRiskTest`);
    } else {
      const reviews = Array.isArray(riskTest.companyReviews) ? riskTest.companyReviews : [];
      const acceptableTickers = (Array.isArray(memo?.acceptableExpressionSet) ? memo.acceptableExpressionSet : [])
        .filter((x) => x?.conclusion === 'acceptable')
        .map((x) => x.ticker);
      const reviewByTicker = new Map();

      for (const review of reviews) {
        if (typeof review?.ticker !== 'string' || !review.ticker) {
          errors.push(`${memoPath} adversarial company review requires ticker`);
          continue;
        }
        if (reviewByTicker.has(review.ticker)) {
          errors.push(`${memoPath} duplicates adversarial company review for ${review.ticker}`);
          continue;
        }
        reviewByTicker.set(review.ticker, review);

        try { assertIso(review.searchCutoff, `${memoPath}.adversarialCompanyRiskTest.${review.ticker}.searchCutoff`); }
        catch (error) { errors.push(error.message); }
        if (review?.searchCutoff && memo?.cutoffAt && Date.parse(review.searchCutoff) > Date.parse(memo.cutoffAt)) {
          errors.push(`${memoPath} adversarial company review ${review.ticker} searchCutoff must not be after memo cutoff`);
        }
        if (typeof review?.searchIntent !== 'string' || !review.searchIntent.trim()) {
          errors.push(`${memoPath} adversarial company review ${review.ticker} requires searchIntent`);
        }
        if (typeof review?.strongestCounterThesis !== 'string' || !review.strongestCounterThesis.trim()) {
          errors.push(`${memoPath} adversarial company review ${review.ticker} requires strongestCounterThesis`);
        }

        const requiredLanes = [
          'leverage_liquidity_refinancing',
          'capital_allocation_mna_goodwill',
          'working_capital_cash_conversion',
          'dilution_capex_pledge',
          'governance_internal_control_regulatory',
          'contingent_liabilities_guarantees_litigation',
          'concentration_other_material'
        ];
        const allowedLaneStatuses = [
          'searched_no_material_issue',
          'manageable_issue',
          'material_challenge',
          'unresolved',
          'not_applicable'
        ...
        if (!lanes || typeof lanes !== 'object' || Array.isArray(lanes)) {
          errors.push(`${memoPath} adversarial company review ${review.ticker} requires lanes object`);
        } else {
          for (const laneName of requiredLanes) {
            const lane = lanes[laneName];
            if (!lane || typeof lane !== 'object') {
              errors.push(`${memoPath} adversarial company review ${review.ticker} missing risk lane ${laneName}`);
              continue;
            }
            if (!['applicable', 'not_applicable'].includes(lane.applicability)) {
              errors.push(`${memoPath} adversarial risk lane ${review.ticker}/${laneName} applicability is invalid`);
            }
            if (!allowedLaneStatuses.includes(lane.status)) {
              errors.push(`${memoPath} adversarial risk lane ${review.ticker}/${laneName} status is invalid`);
            }
            if (lane.applicability === 'not_applicable' && lane.status !== 'not_applicable') {
              errors.push(`${memoPath} non-applicable adversarial risk lane ${review.ticker}/${laneName} must use not_applicable status`);
            }
            if (lane.applicability === 'applicable' && lane.status === 'not_applicable') {
              errors.push(`${memoPath} applicable adversarial risk lane ${review.ticker}/${laneName} cannot use not_applicable status`);
            }
            if (typeof lane.rationale !== 'string' || !lane.rationale.trim()) {
              errors.push(`${memoPath} adversarial risk lane ${review.ticker}/${laneName} requires rationale`);
            }
            if (!Array.isArray(lane.searchTrace) || lane.searchTrace.some((x) => typeof x !== 'string' || !x.trim()) ||
              ...
              errors.push(`${memoPath} applicable adversarial risk lane ${review.ticker}/${laneName} requires non-empty searchTrace`);
            }
            if (!Array.isArray(lane.evidenceRefs) || lane.evidenceRefs.some((x) => typeof x !== 'string' || !x.trim())) {
              errors.push(`${memoPath} adversarial risk lane ${review.ticker}/${laneName} evidenceRefs must be a string array`);
            }
            if (['searched_no_material_issue', 'manageable_issue', 'material_challenge'].includes(lane.status) &&
                lane.evidenceRefs.length === 0) {
              errors.push(`${memoPath} adversarial risk lane ${review.ticker}/${laneName} ${lane.status} requires evidenceRefs`);
            }
          }
        }

        for (const coverageName of ['primarySourceCoverage', 'independentChallengeCoverage']) {
          const coverage = review?.[coverageName];
          if (!coverage || typeof coverage !== 'object') {
            errors.push(`${memoPath} adversarial company review ${review.ticker} requires ${coverageName}`);
            continue;
          }
          if (coverage.attempted !== true) {
            errors.push(`${memoPath} adversarial company review ${review.ticker} ${coverageName}.attempted must be true`);
          }
          if (!['clear', 'credible', 'unavailable', 'insufficient'].includes(coverage.conclusion)) {
            errors.push(`${memoPath} adversarial company review ${review.ticker} ${coverageName}.conclusion is invalid`);
          }
          if (typeof coverage.rationale !== 'string' || !coverage.rationale.trim()) {
            errors.push(`${memoPath} adversarial company review ${review.ticker} ${coverageName}.rationale is required`);
          }
          if (!Array.isArray(coverage.evidenceRefs) || coverage.evidenceRefs.some((x) => typeof x !== 'string' || !x.trim())) {
            errors.push(`${memoPath} adversarial company review ${review.ticker} ${coverageName}.evidenceRefs must be a string array`);
          }
          if (['clear', 'credible'].includes(coverage.conclusion) && coverage.evidenceRefs.length === 0) {
            errors.push(`${memoPath} adversarial company review ${review.ticker} ${coverageName} clear/credible requires evidenceRefs`);
          }
        }

        const impacts = review?.gateImpacts;
        const impactFields = [
          'qualityFloor',
        ...
        if (!impacts || typeof impacts !== 'object') {
          errors.push(`${memoPath} adversarial company review ${review.ticker} requires gateImpacts`);
        } else {
          for (const field of impactFields) {
            if (!['unchanged', 'weakened', 'failed', 'unresolved'].includes(impacts[field])) {
              errors.push(`${memoPath} adversarial company review ${review.ticker} gateImpacts.${field} is invalid`);
            }
          }
        }

        if (!['complete', 'partial', 'insufficient'].includes(review?.searchCompleteness)) {
          errors.push(`${memoPath} adversarial company review ${review.ticker} searchCompleteness is invalid`);
        }
        if (!['no_new_blocker', 'manageable_but_reduces_confidence', 'primary_blocked', 'unresolved'].includes(review?.conclusion)) {
          errors.push(`${memoPath} adversarial company review ${review.ticker} conclusion is invalid`);
        }
      }

      for (const ticker of acceptableTickers) {
        if (!reviewByTicker.has(ticker)) {
          errors.push(`${memoPath} v1.7 requires adversarial company review for acceptable expression ${ticker}`);
        }
      }

      if (typeof riskTest?.rationale !== 'string' || !riskTest.rationale.trim()) {
        errors.push(`${memoPath} adversarialCompanyRiskTest.rationale is required`);
      }
      if (!['complete_no_primary_blocker', 'partial_or_unresolved', 'primary_blocked'].includes(riskTest?.overallConclusion)) {
        errors.push(`${memoPath} adversarialCompanyRiskTest.overallConclusion is invalid`);
      }

      const primary = manifest.primarySelectionStates.includes(memo.selectionState);
      if (primary) {
        if (riskTest.overallConclusion !== 'complete_no_primary_blocker') {
          errors.push(`${memoPath} High-priority v1.7 selection requires complete_no_primary_blocker adversarial conclusion`);
        }
        for (const ticker of acceptableTickers) {
          const review = reviewByTicker.get(ticker);
          if (!review) continue;
          if (review.searchCompleteness !== 'complete') {
            errors.push(`${memoPath} High-priority v1.7 expression ${ticker} requires complete adversarial search`);
          }
          if (!['clear', 'credible'].includes(review?.primarySourceCoverage?.conclusion)) {
            errors.push(`${memoPath} High-priority v1.7 expression ${ticker} requires clear/credible primary-source risk coverage`);
          }
          if (!['no_new_blocker', 'manageable_but_reduces_confidence'].includes(review.conclusion)) {
            errors.push(`${memoPath} High-priority v1.7 expression ${ticker} cannot have blocked/unresolved adversarial conclusion`);
          }
          const impacts = review?.gateImpacts ?? {};
          if (Object.values(impacts).some((x) => x === 'failed' || x === 'unresolved')) {
            errors.push(`${memoPath} High-priority v1.7 expression ${ticker} cannot retain failed/unresolved gate impact after adversarial search`);
          }
        }
      }
    }
...
function validateMemoForSelection(memo, selection, manifest) {
  ...
  if (isV12 && memo?.selectionState !== 'No selection') {
    ...
    const isV13 = ['1.3.0', '1.4.0', '1.5.0', '1.6.0', '1.7.0'].includes(manifest?.skill?.version);
