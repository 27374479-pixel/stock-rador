...
const { evaluateRun, readJson, verifyLock } = require('./backtest-core.cjs');

const lockPath = process.argv[2];
const pricesPath = process.argv[3];
if (!lockPath || !pricesPath) {
  console.error('Usage: node scripts/evaluate-skill-run.cjs <lock.json> <prices.json> [outcome.json]');
  process.exit(2);
}
