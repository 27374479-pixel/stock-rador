...
const P=JSON.parse(fs.readFileSync(path.join(__dirname,'..','research','skill_v33_2025_12_research.json'),'utf8'));
const OUT=path.join(__dirname,'..','research','skill_v33_2025_12_prefreeze.json');
const Q=[
...
async function main(){
 ...
 for(const s of out.signals)for(const c of s.candidates??[]){
   const [q,r]=await Promise.all([
     fetchJson(url(c.symbol,'2025-02-01',s.signalDate,'qfq')),
     fetchJson(url(c.symbol,'2025-02-01',s.signalDate,'none'))
   ]);
   ...
   const raw=rr.filter(x=>x.date<=s.signalDate);
   if(!raw.length)throw new Error(`no pre-signal price for ${c.ticker}`);
   const close=raw.at(-1).close,f=FUND[c.ticker],mc=close*f.sharesBn;
   c.preSignalPrice={
     maxFetchedDate:s.signalDate,
     actualLastBarDate:raw.at(-1).date,
     close:round(close,3),
     ...
     valuation:{
       ...
       simpleAnnualized9mPE:round(mc/(f.profit9mBn*4/3),2),
       warning:'Simple 9M annualization is context only; it is not a normalized-cycle forecast.'
     }
   };
 }
