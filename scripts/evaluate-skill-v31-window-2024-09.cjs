...
const LOCK=JSON.parse(fs.readFileSync(path.join(__dirname,'..','frozen','skill_v31_2024_09_selection.json'),'utf8'));
const OUT_JSON=path.join(__dirname,'..','research','skill_v31_2024_09_outcomes.json');
const OUT_MD=path.join(__dirname,'..','research','skill_v31_2024_09_outcomes.md');
const H=[20,60,120,250];
const PRICE_START='2024-08-01';
const PRICE_END='2025-10-31';
const BENCH={symbol:'sh000300',ticker:'000300.SH',name:'沪深300'};
const COST=0.001;
const META={'601600.SH':{symbol:'sh601600',name:'中国铝业',limitRate:0.10}};
...
function url(symbol,adj){return `https://web.ifzq.gtimg.cn/appstock/app/fqkline/get?param=${symbol},day,${PRICE_START},${PRICE_END},500,${adj}`;}
function parse(j,symbol,adj,isIndex=false){
  const n=j?.data?.[symbol], key=adj==='qfq'?'qfqday':'day';
  const rs=n?.[key]??(isIndex?n?.day:undefined);
...
function maxDD(bars,entryDate,exitDate,entryPrice){
  const s=bars.filter(b=>b.date>=entryDate&&b.date<=exitDate);
  return s.length?Math.min(...s.map(b=>b.low/entryPrice-1),0):null;
}
async function main(){
  ...
  if(selected.length!==1||selected[0].ticker!=='601600.SH') throw new Error('unexpected frozen candidate set');
  const m=META['601600.SH'];
  ...
  for(const days of H){
    ...
    horizons[String(days)]=r.status==='executed'?{
      status:'executed',entryDate:r.entryDate,exitDate:r.exitDate,netReturn:round(r.netReturn),
      benchmarkReturn:round(r.benchmarkReturn),excessReturn:round(r.excessReturn),
  ...
  const out={schemaVersion:'3.1',windowId:LOCK.windowId,generatedAt:new Date().toISOString(),
    selectionChangedAfterOutcome:false,watchOutcomesOpened:false,
  ...
  fs.writeFileSync(OUT_MD,`# Skill V3.1 September 2024 outcome evaluation
