...
const P=JSON.parse(fs.readFileSync(path.join(__dirname,'..','research','skill_v32_2025_06_research.json'),'utf8'));
const OUT=path.join(__dirname,'..','research','skill_v32_2025_06_prefreeze.json');
const Q=[
"688676.SH","601179.SH","600089.SH","002028.SZ","300308.SZ","300502.SZ","002281.SZ","300327.SZ","688595.SH","603986.SH","603416.SH","300124.SZ","000893.SZ","600096.SH","000921.SZ","000333.SZ","002050.SZ","002543.SZ","603366.SH","301211.SZ","603019.SH","000977.SZ","002837.SZ","301018.SZ","002335.SZ","300394.SZ","603083.SH","300870.SZ","002851.SZ","002364.SZ","002518.SZ","002463.SZ","300045.SZ","301308.SZ","688525.SH","001309.SZ","603993.SH","002714.SZ","601600.SH","002922.SZ","002916.SZ"
];
validateResearchPacket(P,{quarantinedTickers:Q});

const F={
 "601919.SH":{sharesBn:15.633725723,q1ProfitBn:11.694538846,profit2024Bn:49.172465},
 "300748.SZ":{sharesBn:1.372131923,q1ProfitBn:0.160525259}
};
...
async function main(){
 ...
 for(const s of out.signals)for(const c of s.candidates??[]){
   const [q,r]=await Promise.all([fetchJson(url(c.symbol,'2024-08-01',s.signalDate,'qfq')),fetchJson(url(c.symbol,'2024-08-01',s.signalDate,'none'))]);
   ...
   const close=rr.filter(x=>x.date<=s.signalDate).at(-1).close,f=F[c.ticker],mc=close*f.sharesBn;
   const valuation={actualClose:round(close,3),marketCapBn:round(mc,2)};
   if(c.ticker==='601919.SH'){
     valuation.simple2024PE=round(mc/f.profit2024Bn,2);
     valuation.simpleAnnualizedQ1PE=round(mc/(f.q1ProfitBn*4),2);
   }
   if(c.ticker==='300748.SZ') valuation.simpleAnnualizedQ1PE=round(mc/(f.q1ProfitBn*4),2);
