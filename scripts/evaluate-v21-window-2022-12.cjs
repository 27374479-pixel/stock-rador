...
const LOCK = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'frozen', 'v21_2022_12_selection.json'), 'utf8'));
const CONFIG = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'config', 'v21_window_2022_12.json'), 'utf8'));
const OUTPUT_JSON = path.join(__dirname, '..', 'research', 'v21_2022_12_outcomes.json');
const OUTPUT_MD = path.join(__dirname, '..', 'research', 'v21_2022_12_outcomes.md');

const HORIZONS = [20, 60, 120, 250];
const PRICE_START = '2022-11-01';
const PRICE_END = '2024-01-31';
...
async function main() {
  ...
  for (const lead of LOCK.leads) {
    ...
    for (const row of lead.watch ?? []) watched.add(row.ticker);
  }
  for (const ticker of selected.keys()) {
    if (watched.has(ticker)) throw new Error(`ticker cannot be both selected and Watch: ${ticker}`);
  }
  ...
  const leads = LOCK.leads.map((lead) => {
    const rows = (lead.candidateBasket ?? []).map((locked) => {
  ...
  const output = {
    ...
    note: 'Only frozen selected candidates were fetched post-signal. Watch candidates remain outcome-unopened.',
    assumptions: {
      benchmark: BENCHMARK,
      entry: 'T+1 open',
      costPerSide: COST,
...
`;
  ...
  fs.writeFileSync(OUTPUT_MD, `# V2.1 December 2022 outcome evaluation

The selection lock was committed before this evaluator existed. Only frozen selected candidates are evaluated. Watch-candidate futures remain unopened.

Trading assumptions: T+1 open, 0.10% cost per side, adjusted prices for returns, unadjusted daily bars for suspension/one-price price-limit blockers, CSI 300 benchmark.
