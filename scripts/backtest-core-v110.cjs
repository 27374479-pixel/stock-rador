...
function validateOpportunityMemo(memo, memoPath, manifest) {
  ...
  if (['1.3', '1.4'].includes(manifest?.schemaVersion) && memo?.selectionState !== 'No selection') {
    ...
    if (!burden || typeof burden !== 'object') {
      ...
      const primary = manifest.primarySelectionStates.includes(memo.selectionState);
      const v07ResetSubstitute = ['0.7.0', '0.8.0', '0.9.0', '1.0.0', '1.1.0'].includes(manifest?.skill?.version) &&
  ...
  if (['0.7.0', '0.8.0', '0.9.0', '1.0.0', '1.1.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
  ...
  if (['0.8.0', '0.9.0', '1.0.0', '1.1.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
    ...
    if (!timing || typeof timing !== 'object') {
      ...
      if (primary) {
        const v10EngineOverride =
          ['1.0.0', '1.1.0'].includes(manifest?.skill?.version) &&
    ...
    if (!asym || typeof asym !== 'object') {
      ...
      if (primary) {
        const v09TurningPoint = ['0.9.0', '1.0.0', '1.1.0'].includes(manifest?.skill?.version) &&
  ...
  if (['0.9.0', '1.0.0', '1.1.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
  ...
  if (['1.0.0', '1.1.0'].includes(manifest?.skill?.version) && memo?.selectionState !== 'No selection') {
    const primary = manifest.primarySelectionStates.includes(memo.selectionState);
    ...
    if (memo?.timingRoute === 'company_engine_override') {
      if (!override || typeof override !== 'object') {
        ...
        if (!dislocation || typeof dislocation !== 'object') {
          ...
          const isV11 = manifest?.skill?.version === '1.1.0';
        ...
        if (primary) {
          ...
          const allowedOverrideDislocation = manifest?.skill?.version === '1.1.0'
            ? ['de_rated', 'early_turn']
            : ['de_rated', 'mixed'];
          if (!allowedOverrideDislocation.includes(dislocation?.conclusion)) {
            errors.push(`${memoPath} High-priority company-engine override requires ${manifest?.skill?.version === '1.1.0' ? 'de_rated or early_turn' : 'de_rated or mixed'} company price dislocation`);
          }
          for (const name of ['realizedEngine', 'independentFundamentalConfirmation', 'fundamentalPriceDivergence', 'nextCatalystWithinBase']) {
