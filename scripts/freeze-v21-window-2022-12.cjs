...
const CONFIG_PATH = path.join(__dirname, '..', 'config', 'v21_window_2022_12.json');
const OUTPUT_JSON = path.join(__dirname, '..', 'research', 'v21_2022_12_selection.json');
const OUTPUT_MD = path.join(__dirname, '..', 'research', 'v21_2022_12_selection.md');
...
`;
  ...
  fs.writeFileSync(OUTPUT_MD, `# V2.1 December 2022 selection freeze

**No post-signal price data is present.** Every market-data request ends on or before the relevant signal date. Previously viewed-outcome tickers are rejected by code. Watch-candidate future returns are deliberately left unopened.
...
${sections}`);
  ...
  console.log(JSON.stringify(leads.map((lead) => ({
    ...
    candidates: lead.candidates.map((candidate) => ({
      ...
      monetization: candidate.score.monetizationScore
    }))
  })), null, 2));
}
