...
const LOCK=JSON.parse(fs.readFileSync(path.join(__dirname,'..','frozen','skill_v32_2025_03_selection.json'),'utf8'));
const OUT_JSON=path.join(__dirname,'..','research','skill_v32_2025_03_outcomes.json');
const OUT_MD=path.join(__dirname,'..','research','skill_v32_2025_03_outcomes.md');

const H=[20,60,120,250];
const PRICE_START='2025-02-01';
const PRICE_END='2026-05-31';
const COST=0.001;
const BENCH={symbol:'sh000300',ticker:'000300.SH',name:'沪深300'};
const META={'002916.SZ':{symbol:'sz002916',name:'深南电路',limitRate:0.10}};
...
function url(symbol,adj){
  return `https://web.ifzq.gtimg.cn/appstock/app/fqkline/get?param=${symbol},day,${PRICE_START},${PRICE_END},500,${adj}`;
}
function parse(j,symbol,adj,isIndex=false){
  const node=j?.data?.[symbol], key=adj==='qfq'?'qfqday':'day';
...
function maxDD(bars,e,x,p){
  const slice=bars.filter(b=>b.date>=e&&b.date<=x);
  return slice.length?Math.min(...slice.map(b=>b.low/p-1),0):null;
}
...
async function main(){
  ...
  if(selected.length!==1||selected[0].ticker!=='002916.SZ') throw new Error('unexpected frozen candidate set');

  const m=META['002916.SZ'];
  const [qfq,raw,bm]=await Promise.all([
    fetchJson(url(m.symbol,'qfq')),
    fetchJson(url(m.symbol,'none')),
    fetchJson(url(BENCH.symbol,'none'))
  ]);
  ...
  const cells=H.map(d=>{
    const x=horizons[String(d)];
    return x.status==='executed'? `${pct(x.excessReturn)} (DD ${pct(x.maxDrawdownFromEntry)})`:x.status;
  });
  fs.writeFileSync(OUT_MD,`# Skill V3.2 March 2025 outcome evaluation

Only the frozen Candidate was fetched. 澜起科技 and 宝钢股份 Watch futures remain unopened.

| Frozen candidate | 20d excess | 60d excess | 120d excess | 250d excess |
| --- | ---: | ---: | ---: | ---: |
| **深南电路** | ${cells.join(' | ')} |

The pre-outcome thesis classified this as structural multi-year with an expected 120-250 trading-day research horizon.
`);
