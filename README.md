...
`src/strict-event-backtest.js` 提供第一版可复用的日线事件回测核心，用来避免把后见价格空间误当成可成交收益。当前规则故意偏保守：

- 信号日不能成交，入口至少是下一交易日（T+1）；
- 可用 `entryDelayBars` 继续增加显式延迟；
- 停牌/零成交量、一字涨停买入和一字跌停卖出一律不假设成交；
- 涨跌停比例必须来自历史点时数据，不能按今天的板块或 ST 状态倒推；
- 买卖成本按乘法方式计入；
- 可选基准必须按同一实际入口和出口日期对齐后再计算超额收益。

这仍只是日线执行原语。日线 OHLC 无法证明开盘价真实成交，也没有解决 5/10/30 分钟级延迟、盘口容量、历史 ST/N 状态、退市样本和组合层面的资金占用；这些必须在后续样本外验证中继续补齐。
