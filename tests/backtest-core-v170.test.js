...
const { classifyCompanyPriceDislocation, validateOpportunityMemo } = require('../scripts/backtest-core-v170.cjs');

function manifest() {
  return {
    schemaVersion: '1.4',
    runId: 'v170-test',
    skill: { path: 'skills/stock-rador/versions/1.7.0/SKILL.md', version: '1.7.0' },
...
function baseMemo() {
  return {
    ...
    adversarialCompanyRiskTest: {
      companyReviews: [{
        ticker: '000001.SZ',
        searchCutoff: '2024-12-31T13:50:00+08:00',
        searchIntent: 'Try to break the company thesis using financing, capital-allocation, cash-conversion, governance and contingent-liability evidence.',
        lanes: {
          leverage_liquidity_refinancing: { applicability: 'applicable', status: 'searched_no_material_issue', rationale: 'Debt, liquidity and maturity stress checked.', searchTrace: ['debt maturity refinancing liquidity'], evidenceRefs: ['e1'] },
          capital_allocation_mna_goodwill: { applicability: 'applicable', status: 'searched_no_material_issue', rationale: 'M&A, goodwill and impairment checked.', searchTrace: ['acquisition goodwill impairment'], evidenceRefs: ['e1'] },
          working_capital_cash_conversion: { applicability: 'applicable', status: 'searched_no_material_issue', rationale: 'Receivables, inventory and cash conversion checked.', searchTrace: ['receivables inventory operating cash flow'], evidenceRefs: ['e1'] },
          dilution_capex_pledge: { applicability: 'applicable', status: 'searched_no_material_issue', rationale: 'Financing, capex and pledge disclosures checked.', searchTrace: ['equity financing convertible capex pledge'], evidenceRefs: ['e1'] },
          governance_internal_control_regulatory: { applicability: 'applicable', status: 'searched_no_material_issue', rationale: 'Governance and regulatory disclosures checked.', searchTrace: ['internal control exchange inquiry governance'], evidenceRefs: ['e1'] },
          contingent_liabilities_guarantees_litigation: { applicability: 'applicable', status: 'searched_no_material_issue', rationale: 'Guarantees, litigation and contingent obligations checked.', searchTrace: ['guarantee litigation contingent liabilities'], evidenceRefs: ['e1'] },
          concentration_other_material: { applicability: 'applicable', status: 'searched_no_material_issue', rationale: 'Concentration and other material risks checked.', searchTrace: ['customer supplier geographic concentration'], evidenceRefs: ['e1'] }
        },
        primarySourceCoverage: { attempted: true, conclusion: 'clear', rationale: 'Cutoff-valid issuer filing checked directly.', evidenceRefs: ['e1'] },
        independentChallengeCoverage: { attempted: true, conclusion: 'credible', rationale: 'Independent challenge source checked.', evidenceRefs: ['e2'] },
        strongestCounterThesis: 'The strongest counter-thesis is ordinary execution/cycle risk rather than hidden financing or governance stress.',
        gateImpacts: {
          qualityFloor: 'unchanged',
          balanceSheetOrSurvival: 'unchanged',
          downsideContainment: 'unchanged',
          expectationBurden: 'unchanged',
          selectionEdge: 'unchanged'
        },
        searchCompleteness: 'complete',
        conclusion: 'no_new_blocker'
      }],
      overallConclusion: 'complete_no_primary_blocker',
      rationale: 'Adversarial company-risk retrieval was completed before promotion and found no fatal or unresolved blocker.'
    },
...
test('v1.7 keeps the normal sector-confirmed High-priority route', () => {
  assert.deepEqual(validateOpportunityMemo(baseMemo(), 'memo.json', manifest()), []);
});

test('v1.7 allows a strict company-engine override under adverse broad timing', () => {
  assert.deepEqual(validateOpportunityMemo(overrideMemo(), 'memo.json', manifest()), []);
});

test('v1.7 company-engine override requires realized, not merely quantitatively bridged, conversion', () => {
...
test('v1.7 company-engine override cannot use turning-point convexity', () => {
  const m = overrideMemo();
...
test('v1.7 company-engine override requires valuation room, not merely tight', () => {
  const m = overrideMemo();
...
test('v1.7 rejects override when price weakness is attributed to company fundamentals', () => {
  const m = overrideMemo();
...
test('v1.7 mechanically rejects legacy company-price dislocation labels', () => {
  const m = overrideMemo();
...
test('v1.7 sector-confirmed path still fails closed under adverse broad timing', () => {
  const m = baseMemo();
  m.opportunityTimingTest.priceRegime = {
    ...m.opportunityTimingTest.priceRegime,
...
test('v1.7 research_only route cannot be High-priority', () => {
  const m = baseMemo();
...
test('v1.7 override can survive a short-lived broad event when the company engine is multi-quarter', () => {
  const m = overrideMemo();
...
test('v1.7 override rejects a short-lived company engine', () => {
  const m = overrideMemo();
...
test('v1.7 override requires clear, not merely credible, cross-sectional asymmetry', () => {
  const m = overrideMemo();
...
test('v1.7 deterministically distinguishes de-rated, early-turn, late-reversal and sustained-strength states', () => {
...
test('v1.7 allows early-turn company-engine override', () => {
  const m = overrideMemo();
...
test('v1.7 blocks late-reversal company-engine override even when fundamentals remain strong', () => {
  const m = overrideMemo();
  m.companyEngineTimingOverrideTest.companyPriceDislocation = {
...
test('v1.7 blocks sustained-strength company-engine override because the override is not an already-rerated chase route', () => {
  const m = overrideMemo();
  m.companyEngineTimingOverrideTest.companyPriceDislocation = {
    selected20Excess: 0.08,
    selected60Excess: 0.14,
...
test('v1.7 rejects a mislabeled early-turn / late-reversal state', () => {
  const m = overrideMemo();
  m.companyEngineTimingOverrideTest.companyPriceDislocation = {
    selected20Excess: -0.06,
    selected60Excess: 0.27,
...
test('v1.7 blocks High-priority when the thesis only extends near-term earnings but does not improve the frozen path', () => {
  const m = baseMemo();
...
test('v1.7 blocks premium growth when the frozen baseline already capitalizes long-duration exceptional growth', () => {
  const m = baseMemo();
...
test('v1.7 requires an explicit near / next / later market baseline path', () => {
  const m = baseMemo();
...
test('v1.7 allows a de-rated cyclical High-priority case when the thesis credibly improves the full path, not just spot earnings', () => {
  const m = overrideMemo();
...
test('v1.7 Research selection may preserve a mixed expectation path without being silently promoted', () => {
  const m = baseMemo();
...
test('v1.7 allows a High-priority acceptable expression without beating every frozen control', () => {
  const m = baseMemo();
...
test('v1.7 still blocks a clearly inferior expression', () => {
  const m = baseMemo();
  m.selectionState = 'High-priority selection';
  m.hypothesisState = 'High-priority hypothesis';
  m.actionableAt = m.researchReadyAt;
...
test('v1.7 supports up to three acceptable expressions and one optional preferred expression', () => {
  const m = baseMemo();
...
test('v1.7 requires an equity-return archetype', () => {
  const m = baseMemo();
...
test('v1.7 requires hypothesis decomposition audit', () => {
  const m = baseMemo();
...
test('v1.7 blocks selection from an aggregate that still requires decomposition', () => {
  const m = baseMemo();
...
test('v1.7 requires explicit evidence horizon resolution', () => {
  const m = baseMemo();
...
test('v1.7 requires high-absorption diagnostic consistency', () => {
  const m = baseMemo();
...
test('v1.7 audits expression purity against the frozen preferred ticker', () => {
  const m = baseMemo();
...
test('v1.7 allows a strict base-horizon convexity High-priority route', () => {
  assert.deepEqual(validateOpportunityMemo(baseHorizonMemo(), 'memo.json', manifest()), []);
});

test('v1.7 base-horizon route does not waive fresh incremental information', () => {
...
test('v1.7 base-horizon route forces catalyst before re-underwrite', () => {
  const m = baseHorizonMemo();
...
test('v1.7 base-horizon route forces re-underwrite before normalization', () => {
  const m = baseHorizonMemo();
...
test('v1.7 base-horizon route cannot extend re-underwrite beyond base horizon', () => {
  const m = baseHorizonMemo();
...
test('v1.7 base-horizon route refuses adverse broad timing', () => {
  const m = baseHorizonMemo();
  m.opportunityTimingTest.priceRegime = {
    ...m.opportunityTimingTest.priceRegime,
...
test('v1.7 base-horizon route still requires acceptable downside containment', () => {
  const m = baseHorizonMemo();
...
test('v1.7 does not backport base-horizon route before v1.5', () => {
  const m = baseHorizonMemo();
...
test('v1.7 requires a causalStateVariableTest on every memo', () => {
  const m = baseMemo();
  delete m.causalStateVariableTest;
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
...
test('v1.7 allows state-leading-flow Research to continue company mapping', () => {
  const m = stateLeadMemo('Research selection');
  assert.deepEqual(validateOpportunityMemo(m, 'memo.json', manifest()), []);
});

test('v1.7 state-leading-flow requires a clear or credible causal lag bridge', () => {
  const m = stateLeadMemo('Research selection');
...
test('v1.7 No selection cannot use lagging downstream flow as the only veto', () => {
...
test('v1.7 No selection remains valid when state leads flow but a separate fatal blocker is frozen', () => {
  const m = stateLeadMemo('No selection');
  assert.deepEqual(validateOpportunityMemo(m, 'memo.json', manifest()), []);
});

test('v1.7 permits No selection when the upstream state itself is not actionable', () => {
  const m = baseMemo();
...
test('v1.7 rejects inconsistent non-applicable state-variable metadata', () => {
  const m = baseMemo();
  m.causalStateVariableTest.stateLeadDisposition = 'research_only';
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
...
test('v1.7 state-leading-flow requires a resolved upstream opportunity implication', () => {
  const m = stateLeadMemo('Research selection');
...
test('v1.7 inherits the v1.5 base-horizon convexity route', () => {
  const m = baseHorizonMemo();
  assert.deepEqual(validateOpportunityMemo(m, 'memo.json', manifest()), []);
});

test('v1.5 memos remain backward-compatible and are not forced to add the v1.7 state test', () => {
  const m = baseMemo();
  delete m.causalStateVariableTest;
...
test('v1.7 requires an explicit adversarial company-risk pass for a selected company', () => {
  const m = baseMemo();
  delete m.adversarialCompanyRiskTest;
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('requires adversarialCompanyRiskTest')));
});

test('v1.7 requires every acceptable expression to receive its own adversarial review', () => {
  const m = baseMemo();
  m.acceptableExpressionSet.push({
    ticker: '000004.SZ', role: 'acceptable', directExposure: 'credible', earningsConversion: 'credible',
    balanceSheetOrSurvival: 'pass', expectationFit: 'credible', downsideContainment: 'credible',
    conclusion: 'acceptable', rationale: 'Second acceptable expression.', evidenceRefs: ['e1']
  });
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('requires adversarial company review for acceptable expression 000004.SZ')));
});

test('v1.7 High-priority fails closed when adversarial search is incomplete', () => {
  const m = baseMemo();
  m.adversarialCompanyRiskTest.companyReviews[0].searchCompleteness = 'partial';
  m.adversarialCompanyRiskTest.overallConclusion = 'partial_or_unresolved';
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('requires complete_no_primary_blocker')));
  assert.ok(errors.some((x) => x.includes('requires complete adversarial search')));
});

test('v1.7 High-priority cannot retain an unresolved company-risk gate impact', () => {
  const m = baseMemo();
  m.adversarialCompanyRiskTest.companyReviews[0].gateImpacts.balanceSheetOrSurvival = 'unresolved';
  m.adversarialCompanyRiskTest.companyReviews[0].conclusion = 'unresolved';
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('cannot have blocked/unresolved adversarial conclusion')));
  assert.ok(errors.some((x) => x.includes('cannot retain failed/unresolved gate impact')));
});

test('v1.7 Research selection may preserve an unresolved adversarial blocker without pretending it is Primary', () => {
  const m = baseMemo();
  m.selectionState = 'Research selection';
  m.hypothesisState = 'Research hypothesis';
  m.actionableAt = null;
  m.timingRoute = 'research_only';
  m.adversarialCompanyRiskTest.companyReviews[0].searchCompleteness = 'partial';
  m.adversarialCompanyRiskTest.companyReviews[0].primarySourceCoverage.conclusion = 'insufficient';
  m.adversarialCompanyRiskTest.companyReviews[0].primarySourceCoverage.evidenceRefs = [];
  m.adversarialCompanyRiskTest.companyReviews[0].gateImpacts.balanceSheetOrSurvival = 'unresolved';
  m.adversarialCompanyRiskTest.companyReviews[0].conclusion = 'unresolved';
  m.adversarialCompanyRiskTest.overallConclusion = 'partial_or_unresolved';
  assert.deepEqual(validateOpportunityMemo(m, 'memo.json', manifest()), []);
...
test('v1.7 adversarial lane cannot claim no material issue without frozen evidence', () => {
  const m = baseMemo();
  m.adversarialCompanyRiskTest.companyReviews[0].lanes.leverage_liquidity_refinancing.evidenceRefs = [];
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('searched_no_material_issue requires evidenceRefs')));
});
