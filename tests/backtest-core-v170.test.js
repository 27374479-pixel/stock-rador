...
test('v1.7 requires an adversarialEvidenceTest on every selected memo', () => {
  const m = baseMemo();
  delete m.adversarialEvidenceTest;
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('v1.7 selected memo requires adversarialEvidenceTest')));
});

test('v1.7 Primary requires complete adversarial search', () => {
  const m = baseMemo();
...
test('v1.7 Primary requires all four generic company-risk axes', () => {
  const m = baseMemo();
...
test('v1.7 Primary blocks unresolved or material required risk axes', () => {
  const m = baseMemo();
...
test('v1.7 Primary allows moderate counter-risk only when carried into downside caveats', () => {
  const m = baseMemo();
  m.adversarialEvidenceTest.axes.find(
...
test('v1.7 rejects a cosmetic proceed-with-caveats label without moderate risk and downside impact', () => {
  const m = baseMemo();
  m.adversarialEvidenceTest.conclusion = 'proceed_with_caveats';
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
...
test('v1.7 Research may preserve a material adversarial risk instead of forcing a false Primary', () => {
  const m = stateLeadMemo('Research selection');
...
test('v1.7 rejects duplicate adversarial risk axes rather than counting repeated evidence as completeness', () => {
  const m = baseMemo();
...
test('v1.6 memos remain backward-compatible and are not forced to add the v1.7 adversarial test', () => {
  const m = baseMemo();
  delete m.adversarialEvidenceTest;
