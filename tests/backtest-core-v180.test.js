...
const { classifyCompanyPriceDislocation, validateOpportunityMemo } = require('../scripts/backtest-core-v180.cjs');

function manifest() {
  return {
    schemaVersion: '1.4',
    runId: 'v180-test',
    skill: { path: 'skills/stock-rador/versions/1.8.0/SKILL.md', version: '1.8.0' },
...
function baseMemo() {
  return {
    ...
    causalStateVariableTest: {
      applicable: false,
      systemType: 'not_applicable',
      stateFlowRelation: 'not_applicable',
      stateLeadDisposition: 'not_applicable',
      independentBlockers: [],
      conclusion: 'not_applicable'
    },
    companyExceptionSearchTest: {
      applicable: false,
      triggerSignals: [],
      overallConclusion: 'not_applicable'
    },
    adversarialEvidenceTest: {
      searchCompleteness: 'complete',
      axes: [
        {
          axis: 'balance_sheet_liquidity',
          conclusion: 'clear',
          rationale: 'cutoff-valid balance sheet and cash/liquidity evidence were checked',
          evidenceRefs: ['e1']
        },
        {
          axis: 'capital_allocation_mna_goodwill',
          conclusion: 'clear',
          rationale: 'cutoff-valid acquisition, capex and goodwill evidence were checked',
          evidenceRefs: ['e1']
        },
        {
          axis: 'governance_related_party',
          conclusion: 'clear',
          rationale: 'cutoff-valid governance and related-party evidence were checked',
          evidenceRefs: ['e1']
        },
        {
          axis: 'refinancing_dilution_contingent',
          conclusion: 'clear',
          rationale: 'cutoff-valid refinancing, dilution and contingent-obligation evidence were checked',
          evidenceRefs: ['e1']
        }
      ],
      strongestCounterThesis: 'No cutoff-valid company-specific risk is strong enough to overturn the frozen thesis.',
      counterThesisSeverity: 'low',
      thesisImpact: 'none',
      falsifierOrEscalationTrigger: 'Any new filing showing material liquidity, governance, acquisition or refinancing stress.',
      conclusion: 'clear_to_proceed'
    },
...
test('v1.6 keeps the normal sector-confirmed High-priority route', () => {
  assert.deepEqual(validateOpportunityMemo(baseMemo(), 'memo.json', manifest()), []);
});

test('v1.6 allows a strict company-engine override under adverse broad timing', () => {
  assert.deepEqual(validateOpportunityMemo(overrideMemo(), 'memo.json', manifest()), []);
});

test('v1.6 company-engine override requires realized, not merely quantitatively bridged, conversion', () => {
...
test('v1.6 company-engine override cannot use turning-point convexity', () => {
  const m = overrideMemo();
...
test('v1.6 company-engine override requires valuation room, not merely tight', () => {
  const m = overrideMemo();
...
test('v1.6 rejects override when price weakness is attributed to company fundamentals', () => {
  const m = overrideMemo();
...
test('v1.6 mechanically rejects legacy company-price dislocation labels', () => {
  const m = overrideMemo();
...
test('v1.6 sector-confirmed path still fails closed under adverse broad timing', () => {
  const m = baseMemo();
  m.opportunityTimingTest.priceRegime = {
    ...m.opportunityTimingTest.priceRegime,
  ...
  m.opportunityTimingTest.overallConclusion = 'adverse';
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('unless a valid override route is used')));
});

test('v1.6 research_only route cannot be High-priority', () => {
  const m = baseMemo();
...
test('v1.6 override can survive a short-lived broad event when the company engine is multi-quarter', () => {
  const m = overrideMemo();
...
test('v1.6 override rejects a short-lived company engine', () => {
  const m = overrideMemo();
...
test('v1.6 override requires clear, not merely credible, cross-sectional asymmetry', () => {
  const m = overrideMemo();
...
test('v1.6 deterministically distinguishes de-rated, early-turn, late-reversal and sustained-strength states', () => {
...
test('v1.6 allows early-turn company-engine override', () => {
  const m = overrideMemo();
...
test('v1.6 blocks late-reversal company-engine override even when fundamentals remain strong', () => {
  const m = overrideMemo();
  m.companyEngineTimingOverrideTest.companyPriceDislocation = {
...
test('v1.6 blocks sustained-strength company-engine override because the override is not an already-rerated chase route', () => {
  const m = overrideMemo();
  m.companyEngineTimingOverrideTest.companyPriceDislocation = {
    selected20Excess: 0.08,
    selected60Excess: 0.14,
...
test('v1.6 rejects a mislabeled early-turn / late-reversal state', () => {
  const m = overrideMemo();
  m.companyEngineTimingOverrideTest.companyPriceDislocation = {
    selected20Excess: -0.06,
    selected60Excess: 0.27,
...
test('v1.6 blocks High-priority when the thesis only extends near-term earnings but does not improve the frozen path', () => {
  const m = baseMemo();
...
test('v1.6 blocks premium growth when the frozen baseline already capitalizes long-duration exceptional growth', () => {
  const m = baseMemo();
...
test('v1.6 requires an explicit near / next / later market baseline path', () => {
  const m = baseMemo();
...
test('v1.6 allows a de-rated cyclical High-priority case when the thesis credibly improves the full path, not just spot earnings', () => {
  const m = overrideMemo();
...
test('v1.6 Research selection may preserve a mixed expectation path without being silently promoted', () => {
  const m = baseMemo();
...
test('v1.6 allows a High-priority acceptable expression without beating every frozen control', () => {
  const m = baseMemo();
...
test('v1.6 still blocks a clearly inferior expression', () => {
  const m = baseMemo();
  m.selectionState = 'High-priority selection';
  m.hypothesisState = 'High-priority hypothesis';
  m.actionableAt = m.researchReadyAt;
...
test('v1.6 supports up to three acceptable expressions and one optional preferred expression', () => {
  const m = baseMemo();
...
test('v1.6 requires an equity-return archetype', () => {
  const m = baseMemo();
...
test('v1.6 requires hypothesis decomposition audit', () => {
  const m = baseMemo();
...
test('v1.6 blocks selection from an aggregate that still requires decomposition', () => {
  const m = baseMemo();
...
test('v1.6 requires explicit evidence horizon resolution', () => {
  const m = baseMemo();
...
test('v1.6 requires high-absorption diagnostic consistency', () => {
  const m = baseMemo();
...
test('v1.6 audits expression purity against the frozen preferred ticker', () => {
  const m = baseMemo();
...
test('v1.6 allows a strict base-horizon convexity High-priority route', () => {
  assert.deepEqual(validateOpportunityMemo(baseHorizonMemo(), 'memo.json', manifest()), []);
});

test('v1.6 base-horizon route does not waive fresh incremental information', () => {
...
test('v1.6 base-horizon route forces catalyst before re-underwrite', () => {
  const m = baseHorizonMemo();
...
test('v1.6 base-horizon route forces re-underwrite before normalization', () => {
  const m = baseHorizonMemo();
...
test('v1.6 base-horizon route cannot extend re-underwrite beyond base horizon', () => {
  const m = baseHorizonMemo();
...
test('v1.6 base-horizon route refuses adverse broad timing', () => {
  const m = baseHorizonMemo();
  m.opportunityTimingTest.priceRegime = {
    ...m.opportunityTimingTest.priceRegime,
...
test('v1.6 base-horizon route still requires acceptable downside containment', () => {
  const m = baseHorizonMemo();
...
function stateLeadMemo(selectionState = 'Research selection') {
  const m = baseMemo();
  m.selectionState = selectionState;
  m.hypothesisState = selectionState === 'High-priority selection'
    ? 'High-priority hypothesis'
    : 'Research hypothesis';
  m.actionableAt = selectionState === 'High-priority selection'
  ...
  m.causalStateVariableTest = {
    ...
    upstreamState: {
      variable: 'productive capacity / replenishment stock',
      observedDirection: 'falling',
      opportunityImplication: 'positive',
      economicRole: 'determines future supply available after the current release/liquidation phase',
      adjustmentLag: 'months',
      evidenceRefs: ['e1']
    },
    downstreamFlow: {
      variable: 'current spot price / current shipment flow',
      observedDirection: 'falling',
      opportunityImplication: 'negative',
      temporaryDistortion: 'temporary liquidation or destocking increases current supply while reducing future productive state',
      evidenceRefs: ['e2']
    },
    stateFlowRelation: 'state_leads_flow',
    causalLagBridge: {
      conclusion: 'clear',
      rationale: 'the frozen causal mechanism explains why current flow can weaken before future supply tightens',
      evidenceRefs: ['e1', 'e2']
    },
    crossSectionalCorroboration: {
      conclusion: 'credible',
      rationale: 'independent cross-sectional evidence is supportive but not treated as the fundamental proof',
      evidenceRefs: ['e3']
    },
    stateLeadDisposition: selectionState === 'No selection'
      ? 'no_selection_independent_blocker'
      : 'continue_company_mapping',
    independentBlockers: selectionState === 'No selection'
      ? ['no investable listed expression after company mapping']
      : [],
    falsifier: 'the upstream productive state stabilizes or the expected causal lag fails to affect future supply',
    conclusion: 'state_leads_flow'
  };
  return m;
}

test('v1.6 requires a causalStateVariableTest on every memo', () => {
  const m = baseMemo();
  delete m.causalStateVariableTest;
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('v1.6 requires causalStateVariableTest')));
});

test('v1.6 accepts an explicit non-applicable state-variable audit', () => {
  assert.deepEqual(validateOpportunityMemo(baseMemo(), 'memo.json', manifest()), []);
});

test('v1.6 allows state-leading-flow Research to continue company mapping', () => {
  const m = stateLeadMemo('Research selection');
  assert.deepEqual(validateOpportunityMemo(m, 'memo.json', manifest()), []);
});

test('v1.6 state-leading-flow requires a clear or credible causal lag bridge', () => {
  const m = stateLeadMemo('Research selection');
  m.causalStateVariableTest.causalLagBridge.conclusion = 'mixed';
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('state_leads_flow requires clear/credible causalLagBridge')));
});

test('v1.6 No selection cannot use lagging downstream flow as the only veto', () => {
  const m = stateLeadMemo('No selection');
  m.causalStateVariableTest.stateLeadDisposition = 'research_only';
  m.causalStateVariableTest.independentBlockers = [];
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('No selection with state_leads_flow requires no_selection_independent_blocker disposition')));
  assert.ok(errors.some((x) => x.includes('requires at least one independent blocker')));
});

test('v1.6 No selection remains valid when state leads flow but a separate fatal blocker is frozen', () => {
  const m = stateLeadMemo('No selection');
  assert.deepEqual(validateOpportunityMemo(m, 'memo.json', manifest()), []);
});

test('v1.6 permits No selection when the upstream state itself is not actionable', () => {
  const m = baseMemo();
  m.selectionState = 'No selection';
  m.hypothesisState = 'Research hypothesis';
  m.actionableAt = null;
  m.acceptableExpressionSet = [];
  m.matchedControls = [];
  m.selectionComparison = { selectedTicker: null };
  m.causalStateVariableTest = {
    ...
    upstreamState: {
      variable: 'installed productive capacity',
      observedDirection: 'stable',
      opportunityImplication: 'mixed',
      economicRole: 'sets the future supply ceiling',
      adjustmentLag: 'quarters',
      evidenceRefs: ['e1']
    },
    downstreamFlow: {
      variable: 'spot price',
      observedDirection: 'stable',
      opportunityImplication: 'mixed',
      temporaryDistortion: 'no evidenced temporary distortion creates a directional state edge',
      evidenceRefs: ['e2']
    },
    stateFlowRelation: 'aligned',
    causalLagBridge: {
      conclusion: 'insufficient',
      rationale: 'no directional state-to-flow edge is established',
      evidenceRefs: ['e1', 'e2']
    },
    crossSectionalCorroboration: {
      conclusion: 'mixed',
      rationale: 'price breadth does not resolve the missing fundamental edge',
      evidenceRefs: ['e3']
    },
    stateLeadDisposition: 'no_selection_state_not_actionable',
    independentBlockers: [],
    falsifier: 'a new state variable establishes a directional earnings bridge',
    conclusion: 'no_actionable_state_edge'
  };
  assert.deepEqual(validateOpportunityMemo(m, 'memo.json', manifest()), []);
});

test('v1.6 rejects inconsistent non-applicable state-variable metadata', () => {
  const m = baseMemo();
  m.causalStateVariableTest.stateLeadDisposition = 'research_only';
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('non-applicable causalStateVariableTest must use stateLeadDisposition not_applicable')));
});

test('v1.6 state-leading-flow requires a resolved upstream opportunity implication', () => {
  const m = stateLeadMemo('Research selection');
  m.causalStateVariableTest.upstreamState.opportunityImplication = 'unresolved';
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('requires a resolved upstream opportunity implication')));
});

test('v1.6 inherits the v1.5 base-horizon convexity route', () => {
  const m = baseHorizonMemo();
  assert.deepEqual(validateOpportunityMemo(m, 'memo.json', manifest()), []);
});

test('v1.5 memos remain backward-compatible and are not forced to add the v1.6 state test', () => {
  const m = baseMemo();
  delete m.causalStateVariableTest;
  m.skillVersion = '1.5.0';
  const oldManifest = manifest();
  oldManifest.skill.version = '1.5.0';
  oldManifest.skill.path = 'skills/stock-rador/versions/1.5.0/SKILL.md';
  assert.deepEqual(validateOpportunityMemo(m, 'memo.json', oldManifest), []);
});


test('v1.8 requires an adversarialEvidenceTest on every selected memo', () => {
  const m = baseMemo();
...
test('v1.8 Primary requires complete adversarial search', () => {
  const m = baseMemo();
  m.adversarialEvidenceTest.searchCompleteness = 'partial';
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('High-priority selection requires complete adversarial search')));
});

test('v1.8 Primary requires all four generic company-risk axes', () => {
  const m = baseMemo();
  m.adversarialEvidenceTest.axes = m.adversarialEvidenceTest.axes.filter(
    (x) => x.axis !== 'capital_allocation_mna_goodwill'
  );
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('requires adversarial axis capital_allocation_mna_goodwill')));
});

test('v1.8 Primary blocks unresolved or material required risk axes', () => {
  const m = baseMemo();
  m.adversarialEvidenceTest.axes.find(
    (x) => x.axis === 'balance_sheet_liquidity'
  ).conclusion = 'material_risk';
  m.adversarialEvidenceTest.counterThesisSeverity = 'high';
  m.adversarialEvidenceTest.thesisImpact = 'block_primary';
  m.adversarialEvidenceTest.conclusion = 'research_only';
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('balance_sheet_liquidity must be clear or credible_risk')));
  assert.ok(errors.some((x) => x.includes('cannot have high/fatal adversarial counter-thesis')));
  assert.ok(errors.some((x) => x.includes('requires adversarial clear_to_proceed/proceed_with_caveats')));
});

test('v1.8 Primary allows moderate counter-risk only when carried into downside caveats', () => {
  const m = baseMemo();
  m.adversarialEvidenceTest.axes.find(
    (x) => x.axis === 'refinancing_dilution_contingent'
  ).conclusion = 'credible_risk';
  m.adversarialEvidenceTest.counterThesisSeverity = 'moderate';
  m.adversarialEvidenceTest.thesisImpact = 'tighten_downside';
  m.adversarialEvidenceTest.strongestCounterThesis = 'Refinancing remains manageable but raises downside sensitivity.';
  m.adversarialEvidenceTest.conclusion = 'proceed_with_caveats';
  assert.deepEqual(validateOpportunityMemo(m, 'memo.json', manifest()), []);
});

test('v1.8 rejects a cosmetic proceed-with-caveats label without moderate risk and downside impact', () => {
  const m = baseMemo();
  m.adversarialEvidenceTest.conclusion = 'proceed_with_caveats';
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('proceed_with_caveats requires moderate severity and tighten_downside')));
});

test('v1.8 Research may preserve a material adversarial risk instead of forcing a false Primary', () => {
  const m = stateLeadMemo('Research selection');
  m.adversarialEvidenceTest = {
    searchCompleteness: 'partial',
    axes: [{
      axis: 'balance_sheet_liquidity',
      conclusion: 'material_risk',
      rationale: 'liquidity evidence is concerning and further work is required',
      evidenceRefs: ['e1']
    }],
    strongestCounterThesis: 'Balance-sheet stress may prevent the operating thesis from accruing to equity holders.',
    counterThesisSeverity: 'high',
    thesisImpact: 'block_primary',
    falsifierOrEscalationTrigger: 'Refinancing or cash-flow evidence resolves the liquidity concern.',
    conclusion: 'research_only'
  };
  assert.deepEqual(validateOpportunityMemo(m, 'memo.json', manifest()), []);
});

test('v1.8 rejects duplicate adversarial risk axes rather than counting repeated evidence as completeness', () => {
  const m = baseMemo();
  m.adversarialEvidenceTest.axes.push({
    ...m.adversarialEvidenceTest.axes[0],
    evidenceRefs: ['e2']
  });
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('duplicates axis balance_sheet_liquidity')));
});

test('v1.6 memos remain backward-compatible and are not forced to add the v1.8 adversarial test', () => {
  const m = baseMemo();
  delete m.adversarialEvidenceTest;
  m.skillVersion = '1.6.0';
  const oldManifest = manifest();
  oldManifest.skill.version = '1.6.0';
  oldManifest.skill.path = 'skills/stock-rador/versions/1.6.0/SKILL.md';
  assert.deepEqual(validateOpportunityMemo(m, 'memo.json', oldManifest), []);
});


function conflictedNoSelectionMemo() {
  const m = baseMemo();
  m.hypothesisState = 'Research hypothesis';
  m.selectionState = 'No selection';
  m.actionableAt = null;
  m.evidenceHorizonMatrix.conflictType = 'multi_axis_conflict';
  m.evidenceHorizonMatrix.conclusion = 'unresolved';
  m.companyExceptionSearchTest = {
    applicable: true,
    triggerSignals: ['evidence_horizon_multi_axis_conflict'],
    searchCompleteness: 'complete',
    heterogeneityLanes: {
      geography_regulatory_exposure: { applicability: 'applicable', status: 'searched_no_exception', rationale: 'geographic and regulatory differences searched', searchTrace: ['region policy exposure'], evidenceRefs: [] },
      compliance_capacity_allowance: { applicability: 'applicable', status: 'searched_no_exception', rationale: 'compliance and allowed capacity searched', searchTrace: ['compliance capacity allowance'], evidenceRefs: [] },
      product_mix: { applicability: 'applicable', status: 'searched_no_exception', rationale: 'product mix searched', searchTrace: ['product mix'], evidenceRefs: [] },
      cost_curve_input_access: { applicability: 'applicable', status: 'searched_no_exception', rationale: 'cost curve and input access searched', searchTrace: ['cost input access'], evidenceRefs: [] },
      contract_pricing_structure: { applicability: 'applicable', status: 'searched_no_exception', rationale: 'contract/pricing structure searched', searchTrace: ['contract pricing'], evidenceRefs: [] },
      customer_endmarket_mix: { applicability: 'applicable', status: 'searched_no_exception', rationale: 'customer and end-market mix searched', searchTrace: ['customer end market mix'], evidenceRefs: [] },
      working_capital_cash_quality: { applicability: 'applicable', status: 'searched_no_exception', rationale: 'cash conversion searched', searchTrace: ['cash conversion working capital'], evidenceRefs: [] },
      other_material_heterogeneity: { applicability: 'not_applicable', status: 'not_applicable', rationale: 'no additional industry-specific heterogeneity was identified', searchTrace: [], evidenceRefs: [] }
    },
    candidateExceptions: [],
    overallConclusion: 'no_listed_exception_found',
    rationale: 'A complete cross-sectional exception search found no listed company with a sufficiently different cutoff-valid earnings path.',
    falsifierOrEscalationTrigger: 'A cutoff-valid company with a distinct exposure and earnings path is identified.'
  };
  return m;
}

test('v1.8 requires an explicit companyExceptionSearchTest on every memo', () => {
  const m = baseMemo();
  delete m.companyExceptionSearchTest;
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('requires companyExceptionSearchTest')));
});

test('v1.8 permits an explicit non-applicable company-exception audit when no aggregate conflict exists', () => {
  assert.deepEqual(validateOpportunityMemo(baseMemo(), 'memo.json', manifest()), []);
});

test('v1.8 aggregate conflict cannot be finalized as No selection with a non-applicable exception search', () => {
  const m = conflictedNoSelectionMemo();
  m.companyExceptionSearchTest = { applicable: false, triggerSignals: [], overallConclusion: 'not_applicable' };
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('aggregate conflict requires applicable companyExceptionSearchTest')));
});

test('v1.8 conflicted aggregate No selection requires complete company-exception search', () => {
  const m = conflictedNoSelectionMemo();
  m.companyExceptionSearchTest.searchCompleteness = 'partial';
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('requires complete company-exception search')));
});

test('v1.8 credible company exception blocks premature aggregate No selection', () => {
  const m = conflictedNoSelectionMemo();
  m.companyExceptionSearchTest.heterogeneityLanes.product_mix.status = 'credible_exception';
  m.companyExceptionSearchTest.heterogeneityLanes.product_mix.evidenceRefs = ['e1'];
  m.companyExceptionSearchTest.candidateExceptions = [{
    ticker: '000004.SZ',
    exceptionMechanism: 'Product mix produces a different earnings path from the conflicted aggregate.',
    directExposure: 'clear',
    earningsPathDifferentiation: 'credible',
    disposition: 'research_candidate',
    rationale: 'Cutoff-valid company evidence supports continued company-level research.',
    evidenceRefs: ['e1']
  }];
  m.companyExceptionSearchTest.overallConclusion = 'exception_candidate_found';
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('cannot terminate while a company exception remains Research candidate')));
});

test('v1.8 allows a conflicted aggregate to continue as Research when a credible company exception is found', () => {
  const m = conflictedNoSelectionMemo();
  m.selectionState = 'Research selection';
  m.hypothesisState = 'High-priority hypothesis';
  m.timingRoute = 'research_only';
  m.acceptableExpressionSet = [{
    ...
    conclusion: 'acceptable', rationale: 'Company exception remains Research pending full underwriting.', evidenceRefs: ['e1']
  }];
  m.selectionComparison = { selectedTicker: '000004.SZ', pairwise: [], selectionEdgeConclusion: 'mixed', rationale: 'Exception is credible but not Primary-ready.' };
  m.crossSectionalAsymmetryTest.selectedTicker = '000004.SZ';
  m.expressionPurityTest.preferredTicker = '000004.SZ';
  m.adversarialEvidenceTest = {
    searchCompleteness: 'partial',
    axes: [{axis:'balance_sheet_liquidity', conclusion:'unresolved', rationale:'Research-stage risk search is incomplete.', evidenceRefs:['e1']}],
    strongestCounterThesis:'Company exception may be offset by company-specific risk.',
    counterThesisSeverity:'high', thesisImpact:'block_primary',
    falsifierOrEscalationTrigger:'Complete company-risk underwriting.',
    conclusion:'research_only'
  };
  m.companyExceptionSearchTest.heterogeneityLanes.product_mix.status = 'credible_exception';
  m.companyExceptionSearchTest.heterogeneityLanes.product_mix.evidenceRefs = ['e1'];
  m.companyExceptionSearchTest.candidateExceptions = [{
    ticker:'000004.SZ', exceptionMechanism:'Different product mix changes earnings path.', directExposure:'clear',
    earningsPathDifferentiation:'credible', disposition:'research_candidate',
    rationale:'Continue company-level Research.', evidenceRefs:['e1']
  }];
  m.companyExceptionSearchTest.overallConclusion = 'exception_candidate_found';
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
...
test('v1.8 company-exception recall does not waive inherited Primary timing gates', () => {
  const m = baseMemo();
  m.evidenceHorizonMatrix.conflictType = 'company_sector_conflict';
  m.companyExceptionSearchTest = conflictedNoSelectionMemo().companyExceptionSearchTest;
  m.companyExceptionSearchTest.triggerSignals = ['evidence_horizon_company_sector_conflict'];
  m.companyExceptionSearchTest.heterogeneityLanes.product_mix.status = 'credible_exception';
  m.companyExceptionSearchTest.heterogeneityLanes.product_mix.evidenceRefs = ['e1'];
  m.companyExceptionSearchTest.candidateExceptions = [{
    ticker:'000001.SZ', exceptionMechanism:'Different product mix changes earnings path.', directExposure:'clear',
    earningsPathDifferentiation:'clear', disposition:'research_candidate', rationale:'Exception found.', evidenceRefs:['e1']
  }];
  m.companyExceptionSearchTest.overallConclusion = 'exception_candidate_found';
  m.opportunityTimingTest.priceRegime = {
    ...m.opportunityTimingTest.priceRegime,
    lookback20Excess:-0.10, lookback60Excess:-0.08, positiveBreadth20:0.2, positiveBreadth60:0.2,
    conclusion:'adverse'
  };
  m.opportunityTimingTest.overallConclusion = 'adverse';
  const errors = validateOpportunityMemo(m, 'memo.json', manifest());
  assert.ok(errors.some((x) => x.includes('High-priority selection requires favorable opportunity timing')));
});
