...
const { classifyCompanyPriceDislocation, validateOpportunityMemo } = require('../scripts/backtest-core-v140.cjs');

function manifest() {
  return {
    schemaVersion: '1.4',
    runId: 'v140-test',
    skill: { path: 'skills/stock-rador/versions/1.4.0/SKILL.md', version: '1.4.0' },
...
test('v1.4 keeps the normal sector-confirmed High-priority route', () => {
  assert.deepEqual(validateOpportunityMemo(baseMemo(), 'memo.json', manifest()), []);
});

test('v1.4 allows a strict company-engine override under adverse broad timing', () => {
  assert.deepEqual(validateOpportunityMemo(overrideMemo(), 'memo.json', manifest()), []);
});

test('v1.4 company-engine override requires realized, not merely quantitatively bridged, conversion', () => {
...
test('v1.4 company-engine override cannot use turning-point convexity', () => {
  const m = overrideMemo();
...
test('v1.4 company-engine override requires valuation room, not merely tight', () => {
  const m = overrideMemo();
...
test('v1.4 rejects override when price weakness is attributed to company fundamentals', () => {
  const m = overrideMemo();
...
test('v1.4 mechanically rejects legacy company-price dislocation labels', () => {
  const m = overrideMemo();
...
test('v1.4 sector-confirmed path still fails closed under adverse broad timing', () => {
  const m = baseMemo();
  m.opportunityTimingTest.priceRegime = {
    ...m.opportunityTimingTest.priceRegime,
...
test('v1.4 research_only route cannot be High-priority', () => {
  const m = baseMemo();
...
test('v1.4 override can survive a short-lived broad event when the company engine is multi-quarter', () => {
  const m = overrideMemo();
...
test('v1.4 override rejects a short-lived company engine', () => {
  const m = overrideMemo();
...
test('v1.4 override requires clear, not merely credible, cross-sectional asymmetry', () => {
  const m = overrideMemo();
...
test('v1.4 deterministically distinguishes de-rated, early-turn, late-reversal and sustained-strength states', () => {
...
test('v1.4 allows early-turn company-engine override', () => {
  const m = overrideMemo();
...
test('v1.4 blocks late-reversal company-engine override even when fundamentals remain strong', () => {
  const m = overrideMemo();
  m.companyEngineTimingOverrideTest.companyPriceDislocation = {
...
test('v1.4 blocks sustained-strength company-engine override because the override is not an already-rerated chase route', () => {
  const m = overrideMemo();
  m.companyEngineTimingOverrideTest.companyPriceDislocation = {
    selected20Excess: 0.08,
    selected60Excess: 0.14,
...
test('v1.4 rejects a mislabeled early-turn / late-reversal state', () => {
  const m = overrideMemo();
  m.companyEngineTimingOverrideTest.companyPriceDislocation = {
    selected20Excess: -0.06,
    selected60Excess: 0.27,
...
test('v1.4 blocks High-priority when the thesis only extends near-term earnings but does not improve the frozen path', () => {
  const m = baseMemo();
...
test('v1.4 blocks premium growth when the frozen baseline already capitalizes long-duration exceptional growth', () => {
  const m = baseMemo();
...
test('v1.4 requires an explicit near / next / later market baseline path', () => {
  const m = baseMemo();
...
test('v1.4 allows a de-rated cyclical High-priority case when the thesis credibly improves the full path, not just spot earnings', () => {
  const m = overrideMemo();
...
test('v1.4 Research selection may preserve a mixed expectation path without being silently promoted', () => {
  const m = baseMemo();
...
test('v1.4 allows a High-priority acceptable expression without beating every frozen control', () => {
  const m = baseMemo();
...
test('v1.4 still blocks a clearly inferior expression', () => {
  const m = baseMemo();
  m.selectionState = 'High-priority selection';
  m.hypothesisState = 'High-priority hypothesis';
  m.actionableAt = m.researchReadyAt;
...
test('v1.4 supports up to three acceptable expressions and one optional preferred expression', () => {
  const m = baseMemo();
...
test('v1.4 requires an equity-return archetype', () => {
  const m = baseMemo();
...
test('v1.4 requires hypothesis decomposition audit', () => {
  const m = baseMemo();
...
test('v1.4 blocks selection from an aggregate that still requires decomposition', () => {
  const m = baseMemo();
...
test('v1.4 requires explicit evidence horizon resolution', () => {
  const m = baseMemo();
...
test('v1.4 requires high-absorption diagnostic consistency', () => {
  const m = baseMemo();
...
test('v1.4 audits expression purity against the frozen preferred ticker', () => {
  const m = baseMemo();
