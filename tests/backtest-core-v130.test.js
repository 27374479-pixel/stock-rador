...
const { classifyCompanyPriceDislocation, validateOpportunityMemo } = require('../scripts/backtest-core-v130.cjs');

function manifest() {
  return {
    schemaVersion: '1.4',
    runId: 'v130-test',
    skill: { path: 'skills/stock-rador/versions/1.3.0/SKILL.md', version: '1.3.0' },
...
test('v1.3 keeps the normal sector-confirmed High-priority route', () => {
  assert.deepEqual(validateOpportunityMemo(baseMemo(), 'memo.json', manifest()), []);
});

test('v1.3 allows a strict company-engine override under adverse broad timing', () => {
  assert.deepEqual(validateOpportunityMemo(overrideMemo(), 'memo.json', manifest()), []);
});

test('v1.3 company-engine override requires realized, not merely quantitatively bridged, conversion', () => {
...
test('v1.3 company-engine override cannot use turning-point convexity', () => {
  const m = overrideMemo();
...
test('v1.3 company-engine override requires valuation room, not merely tight', () => {
  const m = overrideMemo();
...
test('v1.3 rejects override when price weakness is attributed to company fundamentals', () => {
  const m = overrideMemo();
...
test('v1.3 mechanically rejects legacy company-price dislocation labels', () => {
  const m = overrideMemo();
...
test('v1.3 sector-confirmed path still fails closed under adverse broad timing', () => {
  const m = baseMemo();
  m.opportunityTimingTest.priceRegime = {
    ...m.opportunityTimingTest.priceRegime,
...
test('v1.3 research_only route cannot be High-priority', () => {
  const m = baseMemo();
...
test('v1.3 override can survive a short-lived broad event when the company engine is multi-quarter', () => {
  const m = overrideMemo();
...
test('v1.3 override rejects a short-lived company engine', () => {
  const m = overrideMemo();
...
test('v1.3 override requires clear, not merely credible, cross-sectional asymmetry', () => {
  const m = overrideMemo();
...
test('v1.3 deterministically distinguishes de-rated, early-turn, late-reversal and sustained-strength states', () => {
...
test('v1.3 allows early-turn company-engine override', () => {
  const m = overrideMemo();
...
test('v1.3 blocks late-reversal company-engine override even when fundamentals remain strong', () => {
  const m = overrideMemo();
  m.companyEngineTimingOverrideTest.companyPriceDislocation = {
...
test('v1.3 blocks sustained-strength company-engine override because the override is not an already-rerated chase route', () => {
  const m = overrideMemo();
  m.companyEngineTimingOverrideTest.companyPriceDislocation = {
    selected20Excess: 0.08,
    selected60Excess: 0.14,
...
test('v1.3 rejects a mislabeled early-turn / late-reversal state', () => {
  const m = overrideMemo();
  m.companyEngineTimingOverrideTest.companyPriceDislocation = {
    selected20Excess: -0.06,
    selected60Excess: 0.27,
...
test('v1.3 blocks High-priority when the thesis only extends near-term earnings but does not improve the frozen path', () => {
  const m = baseMemo();
...
test('v1.3 blocks premium growth when the frozen baseline already capitalizes long-duration exceptional growth', () => {
  const m = baseMemo();
...
test('v1.3 requires an explicit near / next / later market baseline path', () => {
  const m = baseMemo();
...
test('v1.3 allows a de-rated cyclical High-priority case when the thesis credibly improves the full path, not just spot earnings', () => {
  const m = overrideMemo();
...
test('v1.3 Research selection may preserve a mixed expectation path without being silently promoted', () => {
  const m = baseMemo();
...
test('v1.3 allows a High-priority acceptable expression without beating every frozen control', () => {
  const m = baseMemo();
...
test('v1.3 still blocks a clearly inferior expression', () => {
  const m = baseMemo();
  m.selectionState = 'High-priority selection';
  m.hypothesisState = 'High-priority hypothesis';
  m.actionableAt = m.researchReadyAt;
...
test('v1.3 supports up to three acceptable expressions and one optional preferred expression', () => {
  const m = baseMemo();
...
test('v1.3 requires an equity-return archetype', () => {
  const m = baseMemo();
