...
# Stock Rador — AI Research Skill v0.3
...
3. perform broad semantic triage and freeze both promoted and rejected items with reasons;
...
5. compute researchReadyAt/actionableAt from evidence availability and pre-register the
   realization horizon;
6. lock the discovery policy, implementation code, screening decisions, skill and memos;
7. only then fetch subsequent prices/fundamentals;
8. evaluate discovery quality separately from actionable-signal performance;
9. aggregate returns both by ticker and by hypothesis so multiple stocks expressing one
   thesis do not inflate sample size;
10. inspect misses and false positives;
11. modify the skill based on **generalizable failure classes**, not individual winners;
12. evaluate the new version on a different holdout period;
13. maintain a forward sample that is never used for prompt tuning.
...
Preserve the rejected denominator. Do not use sector names as a shortcut for triage.
