...
## V1.7 prospective-Primary adversarial company-risk contract

V1.7 keeps every V1.6 discovery, causal-state, timing, expectation, valuation,
expression-quality and promotion rule unchanged. It adds one research-completeness gate
before a listed expression can remain High-priority:

**the researcher must try to break the company thesis with company-specific evidence at
the same cutoff, rather than treating a strong industry/event thesis as evidence that the
company itself is safe.**

This is a retrieval/completeness rule, not a stricter valuation rule and not a new route to
promotion. A correct economic state, correct value-chain mapping and even realized earnings
can coexist with a bad equity expression when leverage, liquidity, capital allocation,
governance or contingent obligations were not searched with comparable intensity.

### Mandatory adversarialCompanyRiskTest

Every V1.7 memo with a listed-company selection freezes
`adversarialCompanyRiskTest`. It must contain one `companyReviews` entry for every
expression that the memo keeps as `acceptable`.

Each company review records:

- `ticker`;
- `searchCutoff`;
- `searchIntent`: the strongest company-specific reason the positive thesis could fail;
- seven risk lanes:
...
- for every lane:
  ...
  - `status`: searched_no_material_issue / manageable_issue / material_challenge /
    unresolved / not_applicable;
  - rationale;
  - `searchTrace`: the cutoff-valid search/check paths actually attempted;
  - evidence refs when evidence was found;
- `primarySourceCoverage`: whether issuer filings, exchange disclosures, regulator
  documents or another first-party balance-sheet source were checked;
- `independentChallengeCoverage`: whether an independent credit-rating, reputable
  reporting, exchange/regulatory, specialist or other non-company challenge source was
  actively attempted;
- `strongestCounterThesis`;
- `gateImpacts` on quality floor, balance-sheet/survival, downside containment,
  expectation burden and selection edge;
- `searchCompleteness`: complete / partial / insufficient;
- conclusion: no_new_blocker / manageable_but_reduces_confidence / primary_blocked /
  unresolved.

The top-level test also freezes its overall conclusion and rationale.

### Search behavior

The adversarial pass must be genuinely independent of the positive thesis search.

At minimum:

1. inspect the latest cutoff-valid balance sheet / annual or interim filing when available;
2. search specifically for refinancing pressure, maturity concentration, guarantees,
   pledges, dilution and liquidity stress rather than relying on a generic "quality" label;
3. search acquisitions, goodwill, impairment, related-party transactions, governance,
   internal-control, regulatory/exchange inquiries and material litigation when relevant;
4. inspect receivables, inventory, operating cash conversion and major capex where the
   business model makes them meaningful;
5. attempt at least one source lane that is capable of challenging management's own
   framing.

Syndicated copies of one report count as one origin. A company press release that repeats a
positive thesis does not satisfy the independent challenge pass.

The seven lanes are a coverage checklist, not a claim that every risk exists in every
industry. Marking a lane `not_applicable` requires a company-specific rationale. Do not
invent a bank leverage rule for an industrial company, or an industrial working-capital
rule for a business where that measure is economically meaningless.

### Promotion discipline

For a High-priority selection:

- every acceptable expression must have a frozen company review;
- every company review must be `complete`;
- primary-source coverage must be clear or credible;
- the independent challenge lane must have been attempted;
- no gate impact may be failed or unresolved;
- no company review may conclude `primary_blocked` or `unresolved`;
- the top-level adversarial test must conclude `complete_no_primary_blocker`.

A manageable risk does not automatically reject a company. It must flow through the
existing V1.6 quality, balance-sheet, downside, expectation and selection gates. If those
gates still pass after the adverse evidence is incorporated, the expression may remain
High-priority.

An unresolved potentially fatal risk is different: fail closed to Research until the
uncertainty is resolved.

For a Research selection, partial or unresolved adversarial work may be preserved explicitly
as the reason promotion is not yet justified. This lets the system discover an opportunity
without pretending the company is already fully underwritten.

### One-way property

`adversarialCompanyRiskTest` may only keep or downgrade the state produced by the inherited
V1.6 framework. It can never promote a Research idea, waive an existing blocker or create a
new timing/valuation exception.

"No material issue found" is not a fact about the company unless the memo also freezes what
was searched. Absence of retrieved evidence without a search trace is insufficient.
...
- hard-code a debt ratio, goodwill percentage, pledge ratio, cash-flow cutoff or any other
  numeric threshold from one historical loser;
- create paper, commodity, machinery or any sector-specific exception;
- reject all acquisitive or leveraged companies mechanically;
- reinterpret a later price decline as proof that any pre-cutoff risk was fatal;
- use post-cutoff evidence to make the original adversarial search look complete;
- combine this V1.7 change with dated re-underwrite logic.

V1.7 must be frozen first and tested unchanged on a different untouched historical or
forward window. Dynamic re-underwrite is a separate capability and must be evaluated
separately.
