# Stock Rador v1.3 evaluation protocol

## Primary objective

V1.3 evaluates opportunity capture before exact peer ranking.

At the pre-registered base horizon, report:
- opportunity/hypothesis basket benchmark excess;
- directionCaptureRate;
- positiveBasketRate;
- each selected expression's benchmark excess;
- matched-control excess and peer rank as secondary diagnostics.

A selected stock does not need to be the best frozen peer for the opportunity to count as
captured. It must, however, have been an acceptable expression before reveal and must not
have a clear ex-ante fatal flaw.

## Failure classes

- opportunity_wrong
- opportunity_right_expression_acceptable
- opportunity_right_expression_suboptimal
- opportunity_right_expression_wrong
- expression_fatal_flaw_missed
- exact_peer_ranking_miss
- no_selection_correct
- unresolved

## Anti-overfitting

Do not weaken valuation, expectation-path, earnings-conversion, timing, survival or
future-leakage gates merely to improve directionCaptureRate.

# Stock Rador v1.2 evaluation protocol

## What V1.2 is testing

V1.2 tests whether an explicit frozen earnings path improves expectation-burden decisions
without turning valuation into a numeric factor screen.

Development evidence that motivated the rule includes:
- premium-growth cases where realized earnings stayed strong but the market compressed the
  duration premium;
- a polysilicon super-profit case where low current-year PE coexisted with an explicitly
  lower next-year profit baseline;
- a coal counterexample where low PE, de-rating and a durable path thesis produced strong
  base-horizon relative returns.

None of those development windows may validate V1.2.

## Required post-reveal decomposition

For every selected company report:
1. frozen baseline earnings path;
2. frozen thesis path;
3. whether near-term earnings beat the baseline;
4. whether next-period path improved;
5. whether later/normalized path improved;
6. whether duration risk was correctly classified;
7. benchmark and matched-control result at the pre-registered base horizon.

## Diagnostic labels

- full_path_advantage_right
- near_term_right_duration_wrong
- peak_earnings_optical_value
- premium_duration_overcapitalized
- normalized_earnings_floor_underestimated
- path_advantage_missed
- duration_risk_false_positive
- duration_risk_false_negative
- unresolved

## Anti-overfitting

Do not infer a universal valuation cutoff from any named winner or loser.
Do not encode fixed PE, PEG, forecast-growth, price-return or drawdown thresholds.
The gate is relational: thesis path versus the frozen market baseline path.

V1.2 must be frozen before a new untouched holdout is selected.

# Stock Rador v1.1 evaluation protocol

## What v1.1 is testing

V1.1 tests one narrow refinement only:

Does preserving the direction of 20d/60d company-relative price dislocation improve the
company-engine override by distinguishing an **early turn** from a **late reversal**?

The development evidence is frozen:
- 2023Q4 中际旭创: early_turn, V1.0 success.
- 2023Q2 华电国际: late_reversal, V1.0 failure.

Neither case may validate V1.1.

## Required post-reveal decomposition

For every company-engine override report:
1. frozen dislocation state;
2. realized company-engine persistence;
3. base-horizon benchmark excess;
4. matched-control excess and rank;
5. whether the price state was early recognition or post-rerating reversal;
6. whether a new catalyst actually contained incremental earnings information.

## V1.1 failure labels

- early_turn_override_right
- early_turn_override_wrong
- de_rated_override_right
- de_rated_override_wrong
- late_reversal_correctly_blocked
- late_reversal_missed_continuation
- sustained_strength_correctly_routed
- expectation_absorption_misread
- unresolved

## Success criterion

Do not judge V1.1 by one winner. The desired pattern is:
- company-engine override candidates with `de_rated/early_turn` retain useful precision;
- `late_reversal` cases that would have been V1.0 High-priority are prevented from becoming
  Primary when their prior rerating has already consumed the information.

No fixed return magnitude is part of the rule.
