...
# Stock Rador — AI Research Skill v1.9
...
## V1.9 source-linked fresh-incremental escalation contract

V1.9 keeps every V1.8 company-exception, V1.7 adversarial-risk, V1.6 causal-state,
V1.5 base-horizon, expectation, valuation and expression-quality rule.

It adds one narrow extension to the existing `base_horizon_convexity` route for a failure
class replicated across independent V1.8 diagnostics:

**a newly arrived, source-linked incremental driver can create base-horizon convexity even
when the selected company's expectation fit or downside assessment is still mixed, provided
that the ambiguity is explicitly bounded and every other fundamental gate is resolved.**

This is not a generic momentum override, not a "good company" override, and not permission
to chase a continuation of an old narrative.

### Why a separate source-linked test is required

A memo may not simply label its own evidence "fresh". Every V1.9 selected memo freezes
`freshIncrementalEscalationTest`.

When the test is applicable it must record:

- `eventId`: the frozen discovery event cluster;
- `discoveryNovelty`: new / continuation;
- `eventFirstAvailableAt`;
- `baselineAsOfAt`;
- `freshDriverEvidenceRefs`;
- `baselineEvidenceRefs`;
- `boundedRiskContainment`: clear / credible / mixed / insufficient, with rationale and
  evidence refs;
- `riskWindowTradingDays`;
- `exitDiscipline`;
- `falsifier`;
- conclusion: clear_route / credible_route / no_route / not_applicable.

The lock validator must cross-check the event against the frozen source pack. A valid fresh
route requires:

1. the referenced event cluster exists;
2. its frozen `noveltyAssessment` is `new`, never `continuation`;
3. `eventFirstAvailableAt` matches the frozen event cluster;
4. at least one fresh-driver evidence reference is a member source item of that event;
5. the frozen baseline predates the event;
6. baseline evidence itself was available by the baseline timestamp;
7. all fresh evidence remains cutoff-valid.

There is deliberately **no fixed number of calendar days learned from the motivating
historical cases**. Freshness is defined relationally: the driver must be new relative to a
pre-existing frozen baseline and must still be actionable at the cutoff.

### V1.9 fresh-incremental sub-route

A High-priority memo may use this V1.9 sub-route only while retaining
`timingRoute = base_horizon_convexity`.

The existing V1.5 base-horizon requirements remain binding, plus all of the following:

- `freshIncrementalEscalationTest` concludes clear_route or credible_route;
- discovery novelty is `new`;
- earnings conversion is realized or quantitatively bridged inside the base horizon;
- conventional expectation burden is measured and is `room` or `tight`, never
  unresolved or fully priced;
- valuation slack is `room` or `tight`;
- broad price regime is favorable or neutral, never adverse;
- quality floor passes;
- the next catalyst is clear or credible and occurs inside the pre-registered base horizon;
- `highAbsorptionContinuationTest.freshIncrementalDriver` is clear or credible;
- the V1.7 adversarial search is complete and contains no high/fatal or blocking unresolved
  company-risk conclusion;
- bounded risk containment is clear or credible;
- the risk window does not extend beyond the pre-registered base horizon;
- a forced exit/re-underwrite discipline and falsifier are written before reveal;
- no pairwise control is judged clearly superior or insufficient.

Only on this sub-route, an otherwise acceptable expression may retain `mixed`
`expectationFit` and/or `downsideContainment`, and the memo-level downside assessment may
remain `mixed`, **because the ambiguity is time-bounded and independently audited**.
Insufficient exposure, unresolved valuation, unresolved expectation burden, adverse broad
timing, failed quality, speculative/proxy earnings conversion or a clearly inferior
expression remain fatal.

### Continuation signals remain under the old rules

A `continuation` event cannot use the V1.9 escalation sub-route even when:

- the company is high quality;
- recent earnings are excellent;
- valuation looks reasonable;
- monthly data remain strong;
- the old narrative receives another confirming data point.

Such cases must satisfy an inherited V1.8/V1.5 Primary route without the V1.9 relaxation or
remain Research.
...
- change the company-exception trigger;
- convert Research into Primary automatically;
- waive direct exposure or earnings conversion;
- waive expectation measurement or valuation slack;
- waive V1.7 adversarial company-risk review;
- allow adverse broad timing;
- reward the purest thematic expression;
- introduce dynamic re-underwrite;
- create a theme/reflexivity trading route for causally weak proxies.

### Anti-overfitting

The 2017Q2 and 2017Q3 diagnostics may define this failure class and the counterexamples, but
they may not validate V1.9.

Do not:
- hard-code PV, vitamins, aluminum, machinery or any named stock;
- introduce return thresholds from those outcomes;
- use a fixed freshness-day threshold reverse-engineered from the cases;
- relabel a continuation event as new;
- reinterpret any frozen V1.8 memo after its outcome is known.

Freeze V1.9 and validate it unchanged on a completely untouched historical or forward
window before treating this promotion rule as supported.

## V1.8 company-exception recall contract

V1.8 keeps every V1.7 adversarial-risk, V1.6 causal-state, V1.5 timing,
expectation, valuation, expression-quality and promotion rule unchanged.

It adds one narrowly scoped **research-recall** requirement for a different failure class:

**when the aggregate industry state is genuinely conflicted, the Skill must not stop at
"No selection" until it has searched for listed-company exceptions whose economics differ
from the aggregate because of geography/regulation, compliance/capacity allowance,
product mix, cost curve/input access, contract structure, customer/end-market mix, or
cash-quality differences.**

This is not a new buy route. It exists to prevent an aggregate mixed signal from terminating
company mapping too early.

### Mechanical trigger

V1.8 derives an aggregate-conflict trigger when any of the following cutoff-valid memo
states is present:

- `evidenceHorizonMatrix.conflictType = company_sector_conflict`;
- `evidenceHorizonMatrix.conflictType = multi_axis_conflict`;
- `causalStateVariableTest.stateFlowRelation = conflicted_unresolved`.

A triggered memo must freeze `companyExceptionSearchTest` before finalizing
`No selection`.

### Mandatory companyExceptionSearchTest

Every V1.8 memo freezes the test explicitly. When no aggregate-conflict trigger exists, the
test is marked non-applicable rather than silently omitted.

When applicable it records:

- `applicable: true`;
- `triggerSignals`: which existing conflict states activated the search;
- `searchCompleteness: complete / partial / insufficient`;
- eight generic heterogeneity lanes:
...
- each lane freezes:
  ...
  - `status: searched_no_exception / credible_exception / unresolved / not_applicable`;
  - rationale;
  - the actual `searchTrace`;
  - cutoff-valid evidence refs when evidence is found;
- `candidateExceptions`: zero or more listed expressions with:
  - ticker;
  - exception mechanism;
  - direct-exposure conclusion;
  - earnings-path-differentiation conclusion;
  - evidence refs;
  - disposition: `research_candidate / not_actionable / reject`;
  - rationale;
- overall conclusion:
  - `no_listed_exception_found`;
  - `exception_candidate_found`;
  - `exceptions_rejected`;
  - `unresolved`;
  - `not_applicable`;
- rationale and falsifier/escalation trigger.

A credible exception is not "the stock went up while the sector was mixed." It requires a
pre-cutoff economic mechanism explaining why the company has a different future earnings
path from the conflicted aggregate.

### Final No-selection gate

When aggregate conflict is triggered, final `No selection` is allowed only when:

1. company-exception search is complete;
2. all eight heterogeneity lanes are explicitly audited or marked not applicable with a
   company/industry-specific rationale;
3. no listed candidate remains a `research_candidate`;
4. the overall conclusion is `no_listed_exception_found` or `exceptions_rejected`.

If a credible listed exception remains, the process must continue to company-level Research
instead of terminating the hypothesis at the aggregate layer.

### One-way property

This test is a recall mechanism, not a promotion shortcut.

- It may force more company mapping.
- It may surface a `research_candidate`.
- It may convert an otherwise premature aggregate No-selection process into Research.
- It may **not** waive V1.7 adversarial risk, V1.6 causal mapping, timing, valuation,
  expectation burden, earnings conversion, downside containment or expression-quality
  requirements.
- A later High-priority decision must independently pass every inherited gate; the
  exception-search result itself never counts as a Primary gate.

### Search discipline

Do not mechanically search only "sector leader" or "beneficiary." Search why companies can
have different economics inside the same conflicted industry:

- different regulated geography or policy coverage;
- different environmental/compliance status or permitted productive capacity;
- differentiated product mix or downstream exposure;
- lower/higher position on the cost curve or better input access;
- different fixed-price, cost-plus, take-or-pay or spot contract structure;
- different customers/end markets with different demand sensitivity;
- materially different working-capital and cash-conversion quality;
- another explicit, economically causal source of heterogeneity.

The search must remain industry-agnostic. A lane can be not applicable, but that judgment
must be stated rather than assumed.

### Anti-overfitting

The historical case that exposed this failure class may motivate the contract but cannot
validate it.

Do not:

- create a steel, Fangda, southern-producer, winter-curtailment or environmental-exemption
  rule;
- infer company exception from later relative return;
- promote a company because it happens to be outside the regulated geography;
- ignore leverage, pledges, governance, safety, environmental or valuation risks after a
  company exception is found;
- force every conflicted industry to produce a candidate;
- combine this V1.8 change with dated re-underwrite.

V1.8 must be frozen first and tested unchanged on a different untouched window.
